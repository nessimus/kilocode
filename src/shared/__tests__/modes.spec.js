...
describe("isToolAllowedForMode", () => {
    ...
    describe("file restrictions", () => {
        ...
        it("handles partial streaming cases (path only, no content/diff)", () => {
            ...
            expect(isToolAllowedForMode("write_to_file", "architect", [], undefined, {
                path: "test.js",
            })).toBe(true);
        });
        ...
        it("allows architect mode to edit markdown files only", () => {
            ...
            expect(isToolAllowedForMode("apply_diff", "architect", [], undefined, {
                path: "readme.md",
                diff: "- old\n+ new",
            })).toBe(true);
        ...
        it("applies restrictions to all edit tools including search_and_replace and insert_content", () => {
            ...
            expect(isToolAllowedForMode("search_and_replace", "architect", [], undefined, {
                path: "test.md",
                search: "old text",
                replace: "new text",
            })).toBe(true);
            // Test insert_content with matching file
            expect(isToolAllowedForMode("insert_content", "architect", [], undefined, {
                path: "test.md",
                line: "1",
                content: "# New content",
            })).toBe(true);
        ...
        it("applies restrictions to apply_diff with concurrent file edits (MULTI_FILE_APPLY_DIFF experiment)", () => {
            ...
            const xmlArgs = "<args><file><path>test.md</path><diff><content>- old content\\n+ new content</content></diff></file></args>";
            // Should allow markdown files in architect mode
            expect(isToolAllowedForMode("apply_diff", "architect", [], undefined, {
                args: xmlArgs,
            })).toBe(true);
            // Test with non-markdown file - should throw error
            const xmlArgsNonMd = "<args><file><path>test.py</path><diff><content>- old content\\n+ new content</content></diff></file></args>";
            expect(() => isToolAllowedForMode("apply_diff", "architect", [], undefined, {
                args: xmlArgsNonMd,
            })).toThrow(FileRestrictionError);
            expect(() => isToolAllowedForMode("apply_diff", "architect", [], undefined, {
...
describe("FileRestrictionError", () => {
    ...
    it("formats error message with tool name when provided", () => {
        ...
        expect(error.message).toBe("Tool 'write_to_file' in mode 'Markdown Editor' can only edit files matching pattern: \\.md$. Got: test.js");
        expect(error.name).toBe("FileRestrictionError");
    });
    describe("debug mode", () => {
        it("is configured correctly", () => {
            ...
            expect(debugMode).toMatchObject({
                ...
                roleDefinition: "You are an expert software debugger specializing in systematic problem diagnosis and resolution.",
                groups: ["read", "edit", "browser", "command", "mcp"],
            });
            expect(debugMode?.customInstructions).toContain("Reflect on 5-7 different possible sources of the problem, distill those down to 1-2 most likely sources, and then add logs to validate your assumptions. Explicitly ask the user to confirm the diagnosis before fixing the problem.");
        });
    });
    describe("getFullModeDetails", () => {
        ...
        it("combines custom instructions when cwd provided", async () => {
            ...
            expect(addCustomInstructions).toHaveBeenCalledWith(expect.any(String), "Global instructions", "/test/path", "debug", { language: "en" });
        });
    ...
    it("formats error message with both tool name and description when provided", () => {
        const error = new FileRestrictionError("Markdown Editor", "\\.md$", "Markdown files only", "test.js", "apply_diff");
        expect(error.message).toBe("Tool 'apply_diff' in mode 'Markdown Editor' can only edit files matching pattern: \\.md$ (Markdown files only). Got: test.js");
        expect(error.name).toBe("FileRestrictionError");
    });
});
describe("getModeSelection", () => {
    ...
    test("customMode's properties are used if customMode exists, ignoring promptComponent's properties", () => {
        const selection = getModeSelection("code", { roleDefinition: "Prompt Role Only", customInstructions: "Prompt Instructions Only" }, customModesList);
...
//# sourceMappingURL=modes.spec.js.map