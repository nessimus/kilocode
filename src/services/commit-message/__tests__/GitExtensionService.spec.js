...
describe("GitExtensionService", () => {
    ...
    describe("getDiffForChanges", () => {
        it("should generate diffs per file and exclude files properly for staged changes", async () => {
            ...
            const getDiffForChanges = service.getDiffForChanges;
            const result = await getDiffForChanges.call(service, { staged: true });
            expect(mockSpawnSync).toHaveBeenNthCalledWith(1, "git", ["diff", "--name-only", "--cached"], expect.any(Object));
            // Should call git diff for non-excluded files only
            expect(mockSpawnSync).toHaveBeenNthCalledWith(2, "git", ["diff", "--cached", "--", "src/test.ts"], expect.any(Object));
            expect(mockSpawnSync).toHaveBeenNthCalledWith(3, "git", ["diff", "--cached", "--", "src/utils.ts"], expect.any(Object));
            // Should NOT call git diff for package-lock.json (excluded file)
            expect(mockSpawnSync).not.toHaveBeenCalledWith("git", ["diff", "--cached", "--", "package-lock.json"], expect.any(Object));
        ...
        it("should handle file paths with special characters", async () => {
            ...
            expect(mockSpawnSync).toHaveBeenNthCalledWith(2, "git", ["diff", "--cached", "--", "src/file with spaces.ts"], expect.any(Object));
            expect(mockSpawnSync).toHaveBeenNthCalledWith(3, "git", ["diff", "--cached", "--", "src/file'with'quotes.ts"], expect.any(Object));
...
//# sourceMappingURL=GitExtensionService.spec.js.map