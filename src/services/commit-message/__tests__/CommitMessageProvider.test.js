...
describe("CommitMessageProvider", () => {
    ...
    beforeEach(async () => {
        ...
        childProcessMock.execSync = mockExecSync;
        ...
        vi.mocked(singleCompletionHandler).mockResolvedValue("feat(commit): implement conventional commit message generator");
        // Create CommitMessageProvider instance
        commitMessageProvider = new CommitMessageProvider(mockContext, mockOutputChannel);
        commitMessageProvider.gitService = mockGitService;
    });
    afterEach(() => {
        vi.clearAllMocks();
    });
    describe("generateCommitMessage", () => {
        it("should generate a commit message based on staged changes", async () => {
            ...
            expect(vi.mocked(mockGitService.getCommitContext)).toHaveBeenCalledWith(mockChanges, expect.objectContaining({ staged: true }));
        ...
        it("should generate commit message based on unstaged changes when no staged changes exist", async () => {
            ...
            expect(vi.mocked(mockGitService.getCommitContext)).toHaveBeenCalledWith(mockUnstagedChanges, expect.objectContaining({
                staged: false,
            }));
            expect(singleCompletionHandler).toHaveBeenCalled();
            expect(vi.mocked(mockGitService.setCommitMessage)).toHaveBeenCalled();
        ...
        it("should use custom API config when commitMessageApiConfigId is set", async () => {
            const mockChanges = [{ filePath: "file.ts", status: "Modified" }];
            vi.mocked(mockGitService.gatherChanges).mockResolvedValue(mockChanges);
            ...
            commitMessageProvider.providerSettingsManager = mockProviderSettingsManager;
            // Update the ContextProxy mock to return custom config ID
            const { ContextProxy: MockedContextProxy } = (await vi.importMock("../../../core/config/ContextProxy"));
            const mockInstance = MockedContextProxy.instance;
            mockInstance.getValue.mockImplementation((key) => {
                switch (key) {
                    case "commitMessageApiConfigId":
...
//# sourceMappingURL=CommitMessageProvider.test.js.map