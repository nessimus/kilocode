...
export class CommitMessageProvider {
    ...
    async activate() {
        ...
        const disposable = vscode.commands.registerCommand("kilo-code.generateCommitMessage", (commitContext) => this.generateCommitMessage(commitContext));
     ...
     */
    async generateCommitMessage(commitContext) {
        await vscode.window.withProgress({
            ...
            try {
                ...
                const onDiffProgress = (percentage) => {
    ...
    async callAIForCommitMessageWithProgress(gitContextString, progress) {
        ...
        const progressInterval = setInterval(() => {
            ...
            const incrementLimited = Math.max(remainingProgress * remainingProgress * maxIncrement + minIncrement, minIncrement);
     ...
     */
    async callAIForCommitMessage(gitContextString) {
        ...
        if (commitMessageApiConfigId &&
            listApiConfigMeta.find(({ id }) => id === commitMessageApiConfigId)) {
            try {
     ...
     */
    async buildCommitMessagePrompt(gitContextString, customSupportPrompts) {
        // Load custom instructions including rules
        const workspacePath = getWorkspacePath();
        const customInstructions = workspacePath
            ? await addCustomInstructions("", // no mode-specific instructions for commit
            "", // no global custom instructions
            workspacePath, "commit", // mode for commit-specific rules
            {
        ...
        const shouldGenerateDifferentMessage = this.previousGitContext === gitContextString && this.previousCommitMessage !== null;
...
`;
            ...
            const modifiedTemplate = differentMessagePrefix +
                baseTemplate +
                `
...
FINAL REMINDER: Your message MUST be COMPLETELY DIFFERENT from the previous message: "${this.previousCommitMessage}". This is a critical requirement.`;
            return supportPrompt.create("COMMIT_MESSAGE", {
