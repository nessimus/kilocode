...
export class GitExtensionService {
     ...
     */
    configureRepositoryContext(resourceUri) {
    ...
    determineTargetRepository(resourceUri) {
        try {
    ...
    async getDiffForChanges(options) {
     ...
     */
    async getCommitContext(changes, options) {
        const { staged } = options;
        try {
    ...
    copyToClipboardFallback(message) {
        try {
            ...
            vscode.window.showInformationMessage("Commit message copied to clipboard. Paste it into the commit message field.");
        }
