...
Line 5`;
        ...
        for (let minLines = 1; minLines <= 6; minLines++) {
            ...
            for (let i = minLines; i <= 6; i++) {
                ...
                expect(formatted).toContain(`${headerPrefix} ${i === 1 ? "One" : i === 2 ? "Two" : i === 3 ? "Three" : i === 4 ? "Four" : i === 5 ? "Five" : "Six"} line header`);
            }
        }
    });
...
**Enjoy Kilo Code!** Whether you keep it on a short leash or let it roam autonomously, we can't wait to see what you build.`;
        ...
        if (licenseCaptureIndex !== -1 && licenseCaptureIndex + 1 < captures.length) {
            ...
            expect(licenseDefinitionCapture.node.endPosition.row).toBeGreaterThan(content.split("\n").findIndex((line) => line === "---"));
        }
    });
});
...
//# sourceMappingURL=markdownParser.spec.js.map