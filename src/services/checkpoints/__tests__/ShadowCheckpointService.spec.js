...
const initWorkspaceRepo = async ({ workspaceDir, userName = "Kilo Code", userEmail = "support@kilocode.ai", testFileName = "test.txt", textFileContent = "Hello, world!", }) => {
...
describe.each([[RepoPerTaskCheckpointService, "RepoPerTaskCheckpointService"]])("CheckpointService", (klass, prefix) => {
    ...
    describe(`${klass.name}#hasNestedGitRepositories`, () => {
        it("throws error when nested git repositories are detected during initialization", async () => {
            ...
            await expect(service.initShadowGit()).rejects.toThrow("Checkpoints are disabled because nested git repositories were detected in the workspace");
            // Clean up.
            vitest.restoreAllMocks();
    ...
    describe(`${klass.name}#saveCheckpoint with allowEmpty option`, () => {
        ...
        it("logs correct message for allowEmpty option", async () => {
            ...
            const saveCheckpointLogs = logMessages.filter((msg) => msg.includes("starting checkpoint save") && msg.includes("allowEmpty: true"));
            expect(saveCheckpointLogs).toHaveLength(1);
            await testService.saveCheckpoint("Test logging without allowEmpty");
            const defaultLogs = logMessages.filter((msg) => msg.includes("starting checkpoint save") && msg.includes("allowEmpty: false"));
...
//# sourceMappingURL=ShadowCheckpointService.spec.js.map