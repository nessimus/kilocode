 ...
 */
export async function getCommand(cwd, name) {
    ...
    const projectCommand = (await tryLoadCommand(projectSopsDir, name, "project")) ??
        (await tryLoadCommand(projectCommandsDir, name, "project"));
    if (projectCommand) {
        return projectCommand;
    }
    // Check global directory if not found in project
    const globalCommand = (await tryLoadCommand(globalSopsDir, name, "global")) ?? (await tryLoadCommand(globalDir, name, "global"));
    if (globalCommand) {
 ...
 */
export async function getCommandNames(cwd) {
 ...
 */
export function getCommandNameFromFile(filename) {
