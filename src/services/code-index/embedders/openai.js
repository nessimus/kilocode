...
import { MAX_BATCH_TOKENS, MAX_ITEM_TOKENS, MAX_BATCH_RETRIES as MAX_RETRIES, INITIAL_RETRY_DELAY_MS as INITIAL_DELAY_MS, } from "../constants";
 ...
 */
export class OpenAiEmbedder extends OpenAiNativeHandler {
    embeddingsClient;
    defaultModelId;
    /**
        ...
        const processedTexts = queryPrefix
            ? texts.map((text, index) => {
                ...
                if (estimatedTokens > MAX_ITEM_TOKENS) {
                    console.warn(t("embeddings:textWithPrefixExceedsTokenLimit", {
                        index,
                        estimatedTokens,
        ...
        while (remainingTexts.length > 0) {
            ...
            for (let i = 0; i < remainingTexts.length; i++) {
                ...
                if (itemTokens > MAX_ITEM_TOKENS) {
                    console.warn(t("embeddings:textExceedsTokenLimit", {
                        index: i,
                        itemTokens,
                        maxTokens: MAX_ITEM_TOKENS,
                    }));
        ...
        for (let attempts = 0; attempts < MAX_RETRIES; attempts++) {
            try {
                ...
                if (httpError?.status === 429 && hasMoreAttempts) {
                    ...
                    console.warn(t("embeddings:rateLimitRetry", {
                        delayMs,
                        attempt: attempts + 1,
