...
vitest.mock("../../../../i18n", () => ({
    t: (key, params) => {
        const translations = {
            "embeddings:authenticationFailed": "Failed to create embeddings: Authentication failed. Please check your API key.",
...
describe("OpenAICompatibleEmbedder", () => {
    ...
    describe("constructor", () => {
        ...
        it("should throw error when baseUrl is missing", () => {
            expect(() => new OpenAICompatibleEmbedder("", testApiKey, testModelId)).toThrow("embeddings:validation.baseUrlRequired");
        });
        it("should throw error when apiKey is missing", () => {
            expect(() => new OpenAICompatibleEmbedder(testBaseUrl, "", testModelId)).toThrow("embeddings:validation.apiKeyRequired");
        });
        it("should throw error when both baseUrl and apiKey are missing", () => {
            expect(() => new OpenAICompatibleEmbedder("", "", testModelId)).toThrow("embeddings:validation.baseUrlRequired");
        });
    });
    ...
    describe("createEmbeddings", () => {
        ...
        describe("retry logic", () => {
            ...
            it("should not retry on non-rate-limit errors", async () => {
                ...
                await expect(embedder.createEmbeddings(testTexts)).rejects.toThrow("Failed to create embeddings: Authentication failed. Please check your API key.");
                expect(mockEmbeddingsCreate).toHaveBeenCalledTimes(1);
            ...
            it("should throw error immediately on non-retryable errors", async () => {
                ...
                await expect(embedder.createEmbeddings(testTexts)).rejects.toThrow("Failed to create embeddings after 3 attempts: HTTP 500 - Internal server error");
                expect(mockEmbeddingsCreate).toHaveBeenCalledTimes(1);
            });
        });
        /**
        ...
        describe("error handling", () => {
            it("should handle API errors gracefully", async () => {
                ...
                await expect(embedder.createEmbeddings(testTexts)).rejects.toThrow("Failed to create embeddings after 3 attempts: API connection failed");
                expect(console.error).toHaveBeenCalledWith(expect.stringContaining("OpenAI Compatible embedder error"), apiError);
            });
            it("should handle batch processing errors", async () => {
                const testTexts = ["text1", "text2"];
                const batchError = new Error("Batch processing failed");
                mockEmbeddingsCreate.mockRejectedValue(batchError);
                await expect(embedder.createEmbeddings(testTexts)).rejects.toThrow("Failed to create embeddings after 3 attempts: Batch processing failed");
                expect(console.error).toHaveBeenCalledWith(expect.stringContaining("OpenAI Compatible embedder error"), expect.any(Error));
            });
            ...
            it("should provide detailed error message for HTTP errors", async () => {
                ...
                await expect(embedder.createEmbeddings(testTexts)).rejects.toThrow("Failed to create embeddings after 3 attempts: HTTP 400 - Bad request");
            });
        ...
        describe("OpenAI package base64 behavior verification", () => {
            it("should return unparsed base64 when encoding_format is base64", async () => {
                ...
                const returnedFloat32Array = new Float32Array(returnedBuffer.buffer, returnedBuffer.byteOffset, returnedBuffer.byteLength / 4);
        ...
        describe("Azure OpenAI compatibility", () => {
            const azureUrl = "https://myresource.openai.azure.com/openai/deployments/mymodel/embeddings?api-version=2024-02-01";
            ...
            describe("direct HTTP requests", () => {
                it("should use direct fetch for Azure URLs and SDK for base URLs", async () => {
                    ...
                    expect(global.fetch).toHaveBeenCalledWith(azureUrl, expect.objectContaining({
                        method: "POST",
                ...
                it("should handle multiple embeddings and network errors", async () => {
                    ...
                    await expect(embedder.createEmbeddings(["test"])).rejects.toThrow("Failed to create embeddings after 3 attempts");
                });
            });
        });
    });
    describe("URL detection", () => {
        it("should detect Azure deployment URLs as full endpoints", async () => {
            const embedder = new OpenAICompatibleEmbedder("https://myinstance.openai.azure.com/openai/deployments/my-deployment/embeddings?api-version=2023-05-15", "test-key");
            ...
            expect(mockFetch).toHaveBeenCalledWith("https://myinstance.openai.azure.com/openai/deployments/my-deployment/embeddings?api-version=2023-05-15", expect.any(Object));
        });
    ...
    describe("validateConfiguration", () => {
        ...
        it("should validate successfully with full endpoint URL", async () => {
            ...
            expect(result.valid).toBe(true);
            expect(result.error).toBeUndefined();
            expect(mockFetch).toHaveBeenCalledWith(fullUrl, expect.objectContaining({
                method: "POST",
                headers: expect.objectContaining({
                    Authorization: `Bearer ${testApiKey}`,
                }),
...
//# sourceMappingURL=openai-compatible.spec.js.map