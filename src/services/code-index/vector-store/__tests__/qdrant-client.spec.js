...
describe("QdrantVectorStore", () => {
    ...
    beforeEach(() => {
        ...
        QdrantClient.mockImplementation(() => mockQdrantClientInstance);
        createHash.mockReturnValue(mockCreateHashInstance);
        ...
        getWorkspacePath.mockReturnValue(mockWorkspacePath);
    ...
    it("should handle constructor with default URL when none provided", () => {
        const vectorStoreWithDefaults = new QdrantVectorStore(mockWorkspacePath, undefined, mockVectorSize);
    ...
    describe("URL Parsing and Explicit Port Handling", () => {
        describe("HTTPS URL handling", () => {
            ...
            it("should use port 443 for HTTPS URLs with paths and query parameters", () => {
                const vectorStore = new QdrantVectorStore(mockWorkspacePath, "https://example.com/api/v1?key=value", mockVectorSize);
                expect(QdrantClient).toHaveBeenLastCalledWith({
                    host: "example.com",
                    https: true,
                    port: 443,
        ...
        describe("HTTP URL handling", () => {
            ...
            it("should use port 80 for HTTP URLs while preserving paths and query parameters", () => {
                const vectorStore = new QdrantVectorStore(mockWorkspacePath, "http://example.com/api/v1?key=value", mockVectorSize);
                expect(QdrantClient).toHaveBeenLastCalledWith({
                    host: "example.com",
                    https: false,
                    port: 80,
        ...
        describe("Edge cases", () => {
            it("should handle undefined URL with host-based config", () => {
                const vectorStore = new QdrantVectorStore(mockWorkspacePath, undefined, mockVectorSize);
                expect(QdrantClient).toHaveBeenLastCalledWith({
                    host: "localhost",
                    https: false,
                    port: 6333,
    ...
    describe("URL Prefix Handling", () => {
        it("should pass the URL pathname as prefix to QdrantClient if not root", () => {
            const vectorStoreWithPrefix = new QdrantVectorStore(mockWorkspacePath, "http://localhost:6333/some/path", mockVectorSize);
            expect(QdrantClient).toHaveBeenLastCalledWith({
                host: "localhost",
                https: false,
                port: 6333,
        ...
        it("should not pass prefix if the URL pathname is root ('/')", () => {
            const vectorStoreWithoutPrefix = new QdrantVectorStore(mockWorkspacePath, "http://localhost:6333/", mockVectorSize);
            expect(QdrantClient).toHaveBeenLastCalledWith({
                host: "localhost",
                https: false,
                port: 6333,
        ...
        it("should handle HTTPS URL with path as prefix", () => {
            const vectorStoreWithHttpsPrefix = new QdrantVectorStore(mockWorkspacePath, "https://qdrant.ashbyfam.com/api", mockVectorSize);
            expect(QdrantClient).toHaveBeenLastCalledWith({
                host: "qdrant.ashbyfam.com",
                https: true,
                port: 443,
        ...
        it("should normalize URL pathname by removing trailing slash for prefix", () => {
            const vectorStoreWithTrailingSlash = new QdrantVectorStore(mockWorkspacePath, "http://localhost:6333/api/", mockVectorSize);
            expect(QdrantClient).toHaveBeenLastCalledWith({
                host: "localhost",
                https: false,
                port: 6333,
        ...
        it("should normalize URL pathname by removing multiple trailing slashes for prefix", () => {
            const vectorStoreWithMultipleTrailingSlashes = new QdrantVectorStore(mockWorkspacePath, "http://localhost:6333/api///", mockVectorSize);
            expect(QdrantClient).toHaveBeenLastCalledWith({
                host: "localhost",
                https: false,
                port: 6333,
        ...
        it("should handle multiple path segments correctly for prefix", () => {
            const vectorStoreWithMultiSegment = new QdrantVectorStore(mockWorkspacePath, "http://localhost:6333/api/v1/qdrant", mockVectorSize);
            expect(QdrantClient).toHaveBeenLastCalledWith({
                host: "localhost",
                https: false,
                port: 6333,
        ...
        it("should ignore query parameters and fragments when determining prefix", () => {
            const vectorStoreWithQueryParams = new QdrantVectorStore(mockWorkspacePath, "http://localhost:6333/api/path?key=value#fragment", mockVectorSize);
            expect(QdrantClient).toHaveBeenLastCalledWith({
                host: "localhost",
                https: false,
                port: 6333,
    ...
    describe("initialize", () => {
        ...
        it("should log warning for non-404 errors but still create collection", async () => {
            ...
            expect(mockQdrantClientInstance.deleteCollection).not.toHaveBeenCalled();
            expect(mockQdrantClientInstance.createPayloadIndex).toHaveBeenCalledTimes(5);
            expect(console.warn).toHaveBeenCalledWith(expect.stringContaining(`Warning during getCollectionInfo for "${expectedCollectionName}"`), genericError.message);
        ...
        it("should re-throw error from createCollection when no collection initially exists", async () => {
            ...
            await expect(vectorStore.initialize()).rejects.toThrow(/Failed to connect to Qdrant vector database|vectorStore\.qdrantConnectionFailed/);
            expect(mockQdrantClientInstance.getCollection).toHaveBeenCalledTimes(1);
            expect(mockQdrantClientInstance.createCollection).toHaveBeenCalledTimes(1);
            expect(mockQdrantClientInstance.deleteCollection).not.toHaveBeenCalled();
        ...
        it("should log but not fail if payload index creation errors occur", async () => {
            ...
            for (let i = 0; i <= 4; i++) {
                expect(console.warn).toHaveBeenCalledWith(expect.stringContaining(`Could not create payload index for pathSegments.${i}`), indexError.message);
            }
            ;
            console.warn.mockRestore();
        });
    ...
    describe("deleteCollection", () => {
        ...
        it("should log and re-throw error when deletion fails", async () => {
            ...
            expect(vectorStore.collectionExists).toHaveBeenCalledTimes(1);
            expect(mockQdrantClientInstance.deleteCollection).toHaveBeenCalledTimes(1);
            expect(console.error).toHaveBeenCalledWith(`[QdrantVectorStore] Failed to delete collection ${expectedCollectionName}:`, deleteError);
            console.error.mockRestore();
        });
    });
...
//# sourceMappingURL=qdrant-client.spec.js.map