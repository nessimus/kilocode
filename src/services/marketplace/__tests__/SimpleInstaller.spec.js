...
describe("SimpleInstaller", () => {
    ...
    describe("installMode", () => {
        ...
        it("should install mode using CustomModesManager", async () => {
            ...
            const importCall = mockCustomModesManager.importModeWithRules.mock.calls[0];
        ...
        it("should throw error for array content in mode", async () => {
            const arrayContentMode = {
                ...mockModeItem,
                content: ["content1", "content2"],
            };
            await expect(installer.installItem(arrayContentMode, { target: "project" })).rejects.toThrow("Mode content should not be an array");
        });
        it("should throw error for missing content", async () => {
            ...
            await expect(installer.installItem(noContentMode, { target: "project" })).rejects.toThrow("Mode item missing content");
        });
    ...
    describe("installMcp", () => {
        ...
        it("should throw error when mcp.json contains invalid JSON", async () => {
            ...
            await expect(installer.installItem(mockMcpItem, { target: "project" })).rejects.toThrow("Cannot install MCP server: The .kilocode/mcp.json file contains invalid JSON");
    ...
    describe("removeMode", () => {
        ...
        it("should use CustomModesManager to delete mode and clean up rules folder", async () => {
            ...
            vi.mocked(mockCustomModesManager.getCustomModes).mockResolvedValueOnce([
                { slug: "test", name: "Test Mode", source: "project" },
            ]);
        ...
        it("should handle global mode removal with rules cleanup", async () => {
            ...
            vi.mocked(mockCustomModesManager.getCustomModes).mockResolvedValueOnce([
                { slug: "test", name: "Test Mode", source: "global" },
            ]);
        ...
        it("should throw error when mode content is invalid YAML", async () => {
            const invalidModeItem = {
                ...mockModeItem,
                content: "invalid: yaml: content: {",
            };
            await expect(installer.removeItem(invalidModeItem, { target: "project" })).rejects.toThrow("Invalid mode content: unable to parse YAML");
            expect(mockCustomModesManager.deleteCustomMode).not.toHaveBeenCalled();
        });
        it("should throw error when mode has no slug", async () => {
            const noSlugModeItem = {
                ...mockModeItem,
                content: yaml.stringify({
                    name: "Test Mode",
                    roleDefinition: "Test role",
            ...
            await expect(installer.removeItem(noSlugModeItem, { target: "project" })).rejects.toThrow("Mode missing slug identifier");
            expect(mockCustomModesManager.deleteCustomMode).not.toHaveBeenCalled();
        });
        it("should handle array content format", async () => {
            const arrayContentItem = {
                ...mockModeItem,
                content: [
                    {
                        content: yaml.stringify({
        ...
        it("should throw error when CustomModesManager is not available", async () => {
            const installerWithoutManager = new SimpleInstaller(mockContext);
            await expect(installerWithoutManager.removeItem(mockModeItem, { target: "project" })).rejects.toThrow("CustomModesManager is not available");
        });
    });
});
//# sourceMappingURL=SimpleInstaller.spec.js.map