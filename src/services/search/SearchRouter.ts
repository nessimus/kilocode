...
export class SearchRouter {
	...
	private readonly providers: Map<ProviderName, WebSearchProvider>
	...
	public async search(request: WebSearchRequest): Promise<WebSearchSuccess> {
	...
	private getProviderOrder(searchType: WebSearchType): WebSearchProvider[] {
		const orderedNames: ProviderName[] =
			searchType === "deep_research" ? ["tavily", "perplexity", "brave"] : ["tavily", "brave", "perplexity"]

		return orderedNames
			.map((name) => this.providers.get(name))
			.filter((provider): provider is WebSearchProvider => Boolean(provider))
	}
}
...
class TavilySearchProvider implements WebSearchProvider {
	...
	async search(request: WebSearchRequest, signal: AbortSignal): Promise<ProviderSearchResult> {
