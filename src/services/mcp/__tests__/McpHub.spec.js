...
vi.mock("fs/promises", () => ({
    default: {
        ...
        lstat: vi.fn().mockImplementation(() => Promise.resolve({
            isDirectory: () => true,
        })),
...
describe("McpHub", () => {
    ...
    describe("Discriminated union type handling", () => {
        it("should create connected connections with proper type", async () => {
            ...
            const mcpHub = new McpHub(mockProvider);
        ...
        it("should handle type narrowing correctly in callTool", async () => {
            ...
            await expect(mcpHub.callTool("disabled-server", "test-tool", {})).rejects.toThrow("No connection found for server: disabled-server");
        });
    });
    ...
    describe("Null safety improvements", () => {
        it("should handle null client safely in disconnected connections", async () => {
            ...
            await expect(mcpHub.callTool("null-safety-server", "test-tool", {})).rejects.toThrow("No connection found for server: null-safety-server");
            // Try to read resource on disconnected server
            await expect(mcpHub.readResource("null-safety-server", "test-uri")).rejects.toThrow("No connection found for server: null-safety-server");
        });
        ...
        it("should handle missing connections safely", async () => {
            ...
            await expect(mcpHub.callTool("non-existent-server", "test-tool", {})).rejects.toThrow("No connection found for server: non-existent-server");
            await expect(mcpHub.readResource("non-existent-server", "test-uri")).rejects.toThrow("No connection found for server: non-existent-server");
        });
    ...
    describe("server disabled state", () => {
        ...
        it("should prevent calling tools on disabled servers", async () => {
            ...
            await expect(mcpHub.callTool("disabled-server", "some-tool", {})).rejects.toThrow("No connection found for server: disabled-server");
        });
        it("should prevent reading resources from disabled servers", async () => {
            ...
            await expect(mcpHub.readResource("disabled-server", "some/uri")).rejects.toThrow("No connection found for server: disabled-server");
        });
    });
    describe("callTool", () => {
        it("should execute tool successfully", async () => {
            ...
            }, expect.any(Object), expect.objectContaining({ timeout: 60000 }));
        });
        it("should throw error if server not found", async () => {
            await expect(mcpHub.callTool("non-existent-server", "some-tool", {})).rejects.toThrow("No connection found for server: non-existent-server");
        });
        describe("timeout configuration", () => {
            ...
            it("should use default timeout of 60 seconds if not specified", async () => {
                ...
                expect(mockConnection.client.request).toHaveBeenCalledWith(expect.anything(), expect.anything(), expect.objectContaining({ timeout: 60000 }));
            });
            it("should apply configured timeout to tool calls", async () => {
                ...
                mcpHub.connections = [mockConnection];
                await mcpHub.callTool("test-server", "test-tool");
                expect(mockConnection.client.request).toHaveBeenCalledWith(expect.anything(), expect.anything(), expect.objectContaining({ timeout: 120000 }));
            });
        });
        describe("updateServerTimeout", () => {
            ...
            it("should fallback to default timeout when config has invalid timeout", async () => {
                ...
                expect(mockConnectionInvalid.client.request).toHaveBeenCalledWith(expect.anything(), expect.anything(), expect.objectContaining({ timeout: 60000 }));
            });
            ...
            it("should notify webview after updating timeout", async () => {
                ...
                mcpHub.connections = [mockConnection];
                await mcpHub.updateServerTimeout("test-server", 120);
                expect(mockProvider.postMessageToWebview).toHaveBeenCalledWith(expect.objectContaining({
                    type: "mcpServers",
                }));
            });
        });
    ...
    describe("MCP global enable/disable", () => {
        ...
        it("should not connect to servers when MCP is globally disabled", async () => {
            ...
            const mcpHub = new McpHub(disabledMockProvider);
            // Wait for initialization
            await new Promise((resolve) => setTimeout(resolve, 100));
        ...
        it("should connect to servers when MCP is globally enabled", async () => {
            ...
            const mcpHub = new McpHub(enabledMockProvider);
            // Wait for initialization
            await new Promise((resolve) => setTimeout(resolve, 100));
        ...
        it("should handle refreshAllConnections when MCP is disabled", async () => {
            ...
            expect(disabledMockProvider.postMessageToWebview).toHaveBeenCalledWith(expect.objectContaining({
                type: "mcpServers",
            }));
        });
...
//# sourceMappingURL=McpHub.spec.js.map