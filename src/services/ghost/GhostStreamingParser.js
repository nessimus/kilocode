...
export class GhostStreamingParser {
    ...
    extractCompletedChanges() {
        ...
        const changeRegex = /<change>\s*<search>\s*<!\[CDATA\[([\s\S]*?)\]\]>\s*<\/search>\s*<replace>\s*<!\[CDATA\[([\s\S]*?)\]\]>\s*<\/replace>\s*<\/change>/g;
        let match;
        let lastMatchEnd = 0;
    ...
    generateSuggestions(changes) {
        ...
        const needsCursorMarker = changes.some((change) => change.search.includes(CURSOR_MARKER)) && !currentContent.includes(CURSOR_MARKER);
     ...
     */
    findBestMatch(content, searchPattern) {
        // Validate inputs
     ...
     */
    mapNormalizedToOriginalIndex(originalContent, normalizedContent, normalizedIndex) {
        let originalIndex = 0;
        let normalizedPos = 0;
     ...
     */
    sanitizeXMLConservative(buffer) {
...
//# sourceMappingURL=GhostStreamingParser.js.map