...
export class GhostWorkspaceEdit {
    locked = false;
    groupOperationsIntoBlocks = (ops, lineKey) => {
        if (ops.length === 0) {
            return [];
        }
        const blocks = [[ops[0]]];
    ...
    async applyOperations(documentUri, operations, previousOperations) {
    ...
    async revertOperationsPlaceholder(documentUri, operations) {
    ...
    async applyOperationsPlaceholders(documentUri, operations) {
        const workspaceEdit = new vscode.WorkspaceEdit();
        const document = await vscode.workspace.openTextDocument(documentUri);
        if (!document) {
            console.log(`Could not open document: ${documentUri.toString()}`);
    ...
    getActiveFileOperations(suggestions) {
    ...
    async getActiveFileSelectedOperations(suggestions) {
        const empty = {
            documentUri: null,
            operations: [],
            previousOperations: [],
    ...
    async applySelectedSuggestions(suggestions) {
        if (this.locked) {
            return;
        }
        this.locked = true;
    ...
    async revertSuggestionsPlaceholder(suggestions) {
        if (this.locked) {
            return;
        }
        this.locked = true;
    ...
    async applySuggestionsPlaceholders(suggestions) {
        if (this.locked) {
            return;
        }
        this.locked = true;
...
//# sourceMappingURL=GhostWorkspaceEdit.js.map