...
- If errors cascade (one causes others), fix the root cause`;
    ...
    buildUserPrompt(context) {
        ...
        const errorCount = context.diagnostics?.filter((d) => d.severity === vscode.DiagnosticSeverity.Error).length || 0;
        const warningCount = context.diagnostics?.filter((d) => d.severity === vscode.DiagnosticSeverity.Warning).length || 0;
        if (errorCount > 0) {
...
//# sourceMappingURL=ErrorFixStrategy.js.map