...
export class GhostDocumentStore {
    ...
    historyLimit = 3; // Limit the number of snapshots to keep
     ...
     */
    async storeDocument({ document, parseAST = true, bypassDebounce = false, }) {
     ...
     */
    async parseDocumentAST(document) {
        if (!this.astEnabled) {
            return;
        }
        try {
     ...
     */
    analyzeDocumentChanges(oldContent, newContent, filePath) {
        ...
        for (const hunk of patch.hunks) {
            ...
            let consecutiveAdditions = [];
            let consecutiveDeletions = [];
            ...
            for (const line of hunk.lines) {
                ...
                switch (operationType) {
                    ...
                    default:
                        ...
                        this.processConsecutiveChanges(consecutiveAdditions, consecutiveDeletions, currentLineNumber - consecutiveAdditions.length, actions, filePath);
     ...
     */
    processConsecutiveChanges(additions, deletions, lineNumber, actions, filePath) {
     ...
     */
    createAdditionAction(addedLines, lineNumber, filePath) {
     ...
     */
    createDeletionAction(deletedLines, lineNumber, filePath) {
     ...
     */
    createModificationAction(addedLines, deletedLines, lineNumber, filePath) {
     ...
     */
    isFormattingChange(addedText, deletedText) {
...
//# sourceMappingURL=GhostDocumentStore.js.map