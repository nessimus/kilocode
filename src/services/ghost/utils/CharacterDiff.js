 ...
 */
export function calculateDiff(originalText, newText) {
    if (!originalText && !newText)
        return [];
    if (!originalText)
        return [{ start: 0, end: newText.length, type: "added" }];
    if (!newText)
        return [];
    ...
    for (let i = 0; i < wordDiff.length; i++) {
        ...
        if (change.added) {
            ...
            if (nextChange?.added) {
                const { ranges: processedRanges, newPosition } = processWordPair(change.value, nextChange.value, currentPosition);
 ...
 */
function processWordPair(removedWord, addedWord, startPosition) {
 ...
 */
function shouldUseCharacterRefinement(word1, word2) {
 ...
 */
function refineWithCharacterDiff(originalWord, newWord, startPosition) {
...
//# sourceMappingURL=CharacterDiff.js.map