...
import { getShikiTheme, getThemeColors, SUPPORTED_SHIKI_THEMES } from "./ThemeMapper";
...
export function getLanguageForDocument(document) {
    return VS_CODE_TO_SHIKI_LANGUAGE_MAP[document.languageId] || "plaintext";
}
export async function generateHighlightedHtmlWithRanges(code, language, backgroundRanges) {
    if (!highlighter) {
        highlighter = await initializeHighlighter();
    }
    const theme = getShikiTheme();
    const themeColors = getThemeColors();
    const tokensResult = highlighter.codeToTokens(code, { lang: language, theme: theme });
    const html = renderTokensToHtmlWithBackgrounds(tokensResult.tokens, backgroundRanges, themeColors);
    return { html, themeColors };
}
function renderTokensToHtmlWithBackgrounds(lines, backgroundRanges, themeColors) {
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        for (const token of line) {
            ...
            const overlappingRanges = backgroundRanges.filter((range) => range.start < tokenEnd && range.end > tokenStart);
            if (overlappingRanges.length > 0) {
                html += renderTokenWithCharacterRanges(token.content, token.color || themeColors.foreground, tokenStart, backgroundRanges, themeColors);
            }
            else {
...
function renderTokenWithCharacterRanges(text, tokenColor, offset, ranges, themeColors) {
    let html = "";
    let currentIndex = 0;
    const segments = [];
