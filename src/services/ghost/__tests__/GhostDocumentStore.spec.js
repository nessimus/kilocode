...
describe("GhostDocumentStore", () => {
    ...
    beforeEach(() => {
        ...
        documentStore = {
            ...
            storeDocument: vi
                .fn()
                .mockImplementation(async ({ document, parseAST = true, bypassDebounce = false, }) => {
            ...
            parseDocumentAST: vi.fn().mockImplementation(async (document) => {
                const uri = document.uri.toString();
                const item = mockDocumentMap.get(uri);
                if (item) {
                    // Only parse AST for supported file extensions
            ...
            getAST: vi.fn().mockImplementation((documentUri) => {
                const uri = documentUri.toString();
                const item = mockDocumentMap.get(uri);
                return item?.ast;
            }),
            getDocument: vi.fn().mockImplementation((documentUri) => {
                const uri = documentUri.toString();
                return mockDocumentMap.get(uri);
            }),
            needsASTUpdate: vi.fn().mockImplementation((document) => {
                const uri = document.uri.toString();
                const item = mockDocumentMap.get(uri);
                if (!item) {
                    return true;
            ...
            clearAST: vi.fn().mockImplementation((documentUri) => {
                const uri = documentUri.toString();
                const item = mockDocumentMap.get(uri);
                if (item) {
                    item.ast = undefined;
            ...
            getRecentOperations: vi.fn().mockImplementation((document) => {
                if (!document) {
                    return "";
                }
                const uri = document.uri.toString();
    ...
    describe("storeDocument", () => {
        ...
        it("should limit history to historyLimit", async () => {
            ...
            const historyLimit = documentStore.historyLimit;
    ...
    describe("getRecentOperations", () => {
        ...
        it("should handle undefined document", () => {
            const operations = documentStore.getRecentOperations(undefined);
            expect(operations).toBe("");
        });
    });
});
//# sourceMappingURL=GhostDocumentStore.spec.js.map