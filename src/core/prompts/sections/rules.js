...
function getEditingInstructions(diffStrategy) {
    ...
    if (diffStrategy) {
        availableTools.push("apply_diff (for surgical edits - targeted changes to specific lines or functions)", "write_to_file (for creating new files or complete file rewrites)");
    }
    else {
...
export function getRulesSection(cwd, supportsComputerUse, diffStrategy, codeIndexManager, clineProviderState) {
...
${kiloCodeUseMorph ? getMorphEditingInstructions() : getEditingInstructions(diffStrategy)}
