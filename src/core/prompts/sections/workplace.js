...
export function getWorkplacePersonaSection(state) {
    ...
    const lines = ["====", "WORKPLACE CONTEXT", "", `Company: ${companyDisplayName}`];
    if (company.id) {
    ...
    lines.push("- You can suggest or create new employees using the create_employee tool when additional expertise would help. Provide a clear rationale and gather user approval before proceeding.");
    lines.push("- When you need multiple specialists, supply a JSON array in the employees parameter to create several teammates in one tool call.");
    if (!supportingEmployees.length) {
        lines.push("- No additional employees exist yet—consider proposing specialists to round out the team when useful.");
    }
    return lines.join("\n");
}
