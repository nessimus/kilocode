...
function buildPersonaAwareRoleDefinition(
	...
	introParts.push(
		`Speak naturally as ${employee.name}; acknowledge the user, ask how you can help, and wait for their direction before acting or using tools.`,
	)
	...
	if (employee.personality) {
		...
		introParts.push(
			`Let this personality guide your tone, pace, and decision-making so every reply sounds like ${employee.name}.`,
		)
	}
	...
	introParts.push(
		`Keep your focus on ${employee.role} outcomes for ${companyName}. Wait for the user to request work before proposing plans or using tools.`,
	)
	introParts.push(
...
export const SYSTEM_PROMPT = async (
	...
	const activeTeamIds = personaContextForFile
		? (personaContextForFile.company.teams ?? [])
				.filter(
					(team) => !team.deletedAt && (team.employeeIds ?? []).includes(personaContextForFile.employee.id),
				)
				.map((team) => team.id)
	...
	const activeDepartmentIds = personaContextForFile
		? (personaContextForFile.company.departments ?? [])
				.filter(
					(department) =>
						!department.deletedAt && (department.teamIds ?? []).some((id) => activeTeamIds.includes(id)),
				)
