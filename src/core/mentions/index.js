...
export async function openMention(cwd, mention) {
    if (!mention) {
        return;
    }
...
export async function parseMentions(text, cwd, urlContentFetcher, fileContextTracker, rooIgnoreController, showRooIgnoredFiles = false, includeDiagnosticMessages = true, maxDiagnosticMessages = 50, maxReadFileLine) {
        ...
        if (mention.startsWith("http")) {
            ...
            if (launchBrowserError) {
                ...
                try {
                    ...
                    vscode.window.showErrorMessage(t("common:errors.url_fetch_error_with_url", { url: mention, error: localizedErrorMessage }));
...
async function getWorkspaceProblems(cwd, includeDiagnosticMessages = true, maxDiagnosticMessages = 50) {
    ...
    const result = await diagnosticsToProblemsString(diagnostics, [vscode.DiagnosticSeverity.Error, vscode.DiagnosticSeverity.Warning], cwd, includeDiagnosticMessages, maxDiagnosticMessages);
    if (!result) {
 ...
 */
export async function getLatestTerminalOutput() {
