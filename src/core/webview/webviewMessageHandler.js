...
export const webviewMessageHandler = async (provider, message, marketplaceManager) => {
	...
	switch (message.type) {
		...
		case "stopRealtimeSession": {
			...
			provider.log(
				`[*Realtime] stopRealtimeSession request${active ? ` (session=${active.sessionId})` : " (no active session)"}`,
			)
					...
					if (isNewMode) {
						...
						const changedSettings = existingMode
							? Object.keys(message.modeConfig).filter(
									(key) =>
										JSON.stringify(existingMode[key]) !== JSON.stringify(message.modeConfig[key]),
								)
							: []
						if (changedSettings.length > 0) {
							TelemetryService.instance.captureModeSettingChanged(changedSettings[0])
		...
		case "requestCommands": {
			try {
				...
				try {
					...
					provider.log(
						`Error fetching workflow SOPs: ${JSON.stringify(workflowError, Object.getOwnPropertyNames(workflowError ?? {}), 2)}`,
					)
				}
		...
		case "createCommand": {
			try {
				...
				try {
					...
					provider.log(
						`Error refreshing workflow SOPs: ${JSON.stringify(workflowError, Object.getOwnPropertyNames(workflowError ?? {}), 2)}`,
					)
				}
				const documentCommandList = commands.map((command) => ({
					name: command.name,
		...
		case "queueMessage": {
			provider
				.getCurrentTask()
				?.messageQueueService.addMessage(message.text ?? "", message.images, { silent: message.silent })
			break
		}
