...
import { TelemetryEventName, ghostServiceSettingsSchema, // kilocode_change
 } from "@roo-code/types";
...
export const webviewMessageHandler = async (provider, message, marketplaceManager) => {
    ...
    const getRealtimeManager = () => {
        const providerWithRealtime = provider;
    ...
    const findMessageIndices = (messageTs, currentCline) => {
        ...
        const apiConversationHistoryIndex = currentCline.apiConversationHistory.findIndex((msg) => msg.ts === messageTs);
        return { messageIndex, apiConversationHistoryIndex };
    };
    /**
     * Removes the target message and all subsequent messages
     */
    const removeMessagesThisAndSubsequent = async (currentCline, messageIndex, apiConversationHistoryIndex) => {
    ...
    const handleDeleteMessageConfirm = async (messageTs, restoreCheckpoint) => {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Error deleting message: ${error instanceof Error ? error.message : String(error)}`);
        }
    };
    /**
    ...
    const handleEditMessageConfirm = async (messageTs, editedContent, restoreCheckpoint, images) => {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Error editing message: ${error instanceof Error ? error.message : String(error)}`);
        }
    };
    /**
    ...
    debugWebviewLog("received message", message.type, {
        workplaceCompanyId: message?.workplaceCompanyId,
        workplaceEmployeeId: message?.workplaceEmployeeId,
    });
    switch (message.type) {
        case "createCompany": {
            const payload = message.workplaceCompanyPayload;
            if (!payload) {
        ...
        case "createDepartment": {
            const payload = message.workplaceDepartmentPayload;
            if (!payload) {
        ...
        case "updateDepartment": {
            const payload = message.workplaceDepartmentUpdate;
            if (!payload) {
        ...
        case "updateCompany": {
            const payload = message.workplaceCompanyUpdate;
            if (!payload) {
        ...
        case "setCompanyFavorite": {
            const payload = message.workplaceCompanyFavorite;
            if (!payload) {
        ...
        case "deleteCompany": {
            const payload = message.workplaceCompanyDelete;
            if (!payload) {
        ...
        case "createEmployee": {
            const payload = message.workplaceEmployeePayload;
            if (!payload) {
        ...
        case "updateEmployee": {
            const payload = message.workplaceEmployeeUpdate;
            if (!payload) {
        ...
        case "archiveEmployee": {
            const payload = message.workplaceEmployeeArchive;
            if (!payload) {
        ...
        case "createTeam": {
            const payload = message.workplaceTeamPayload;
            if (!payload) {
        ...
        case "updateTeam": {
            const payload = message.workplaceTeamUpdate;
            if (!payload) {
        ...
        case "archiveTeam": {
            const payload = message.workplaceTeamArchive;
            if (!payload) {
        ...
        case "assignTeamToDepartment": {
            const payload = message.workplaceTeamAssignment;
            if (!payload) {
        ...
        case "removeTeamFromDepartment": {
            const payload = message.workplaceTeamDepartmentRemoval;
            if (!payload) {
        ...
        case "assignEmployeeToTeam": {
            const payload = message.workplaceTeamEmployeeAssignment;
            if (!payload) {
        ...
        case "removeEmployeeFromTeam": {
            const payload = message.workplaceTeamEmployeeRemoval;
            if (!payload) {
        ...
        case "setOwnerProfileDefaults": {
            const payload = message.workplaceOwnerProfileDefaults;
            if (!payload) {
        ...
        case "archiveDepartment": {
            const payload = message.workplaceDepartmentArchive;
            if (!payload) {
        ...
        case "createActionItem": {
            const payload = message.workplaceActionItemPayload;
            if (!payload) {
        ...
        case "updateActionItem": {
            const payload = message.workplaceActionItemUpdate;
            if (!payload) {
        ...
        case "deleteActionItem": {
            const payload = message.workplaceActionItemDelete;
            if (!payload) {
        ...
        case "startActionItems": {
            const payload = message.workplaceActionStart;
            if (!payload) {
        ...
        case "updateActionStatus": {
            const payload = message.workplaceActionStatusUpdate;
            if (!payload) {
            ...
            provider.providerSettingsManager
                ...
                if (listApiConfig.length === 1) {
                    ...
                    if (!checkExistKey(listApiConfig[0])) {
                        ...
                        await provider.providerSettingsManager.saveConfig(listApiConfig[0].name ?? "default", apiConfiguration);
                ...
                .catch((error) => provider.log(`Error list api configuration: ${JSON.stringify(error, Object.getOwnPropertyNames(error), 2)}`));
        ...
        case "shareCurrentTask":
            ...
            try {
                ...
                if (result.success && result.shareUrl) {
                    ...
                    const messageKey = visibility === "public"
        ...
        case "flushRouterModels":
            const routerNameFlush = toRouterName(message.text);
        ...
        case "requestRouterModels":
            ...
            const safeGetModels = async (options) => {
                try {
                    ...
                    console.error(`Failed to fetch models in webviewMessageHandler requestRouterModels for ${options.provider}:`, error);
            ...
            const results = await Promise.allSettled(modelFetchPromises.map(async ({ key, options }) => {
                const models = await safeGetModels(options);
                return { key, models }; // key is RouterName here
            }));
            const fetchedRouterModels = {
                ...routerModels,
            ...
            provider.postMessageToWebview({
                type: "routerModels",
                routerModels: fetchedRouterModels,
            });
            break;
        ...
        case "openFile":
            ...
            openFile(filePath, message.values);
            break;
        case "openMention":
        ...
        case "seeNewChanges":
            ...
            if (task && message.payload && message.payload) {
                await seeNewChanges(task, message.payload.commitRange);
            }
            break;
        // kilocode_change end
        ...
        case "toggleToolAlwaysAllow": {
            try {
                await provider
                    .getMcpHub()
                    ?.toggleToolAlwaysAllow(message.serverName, message.source, message.toolName, Boolean(message.alwaysAllow));
            }
            catch (error) {
        ...
        case "toggleToolEnabledForPrompt": {
            try {
                await provider
                    .getMcpHub()
                    ?.toggleToolEnabledForPrompt(message.serverName, message.source, message.toolName, Boolean(message.isEnabled));
            }
            catch (error) {
        ...
        case "toggleMcpServer": {
            try {
                await provider
                    .getMcpHub()
                    ?.toggleServerDisabled(message.serverName, message.disabled, message.source);
            }
            catch (error) {
        ...
        case "testOutboundNotifications": {
            ...
            const email = typeof values.notificationEmail === "string"
                ? values.notificationEmail
                : provider.contextProxy.getValue("notificationEmail");
            const emailPassword = typeof values.notificationEmailAppPassword === "string"
                ? values.notificationEmailAppPassword
                : provider.contextProxy.getValue("notificationEmailAppPassword");
            const smsNumber = typeof values.notificationSmsNumber === "string"
                ? values.notificationSmsNumber
                : provider.contextProxy.getValue("notificationSmsNumber");
            const smsGateway = typeof values.notificationSmsGateway === "string"
                ? values.notificationSmsGateway
                : provider.contextProxy.getValue("notificationSmsGateway");
            const telegramBotToken = typeof values.notificationTelegramBotToken === "string"
                ? values.notificationTelegramBotToken
                : provider.contextProxy.getValue("notificationTelegramBotToken");
            const telegramChatId = typeof values.notificationTelegramChatId === "string"
                ? values.notificationTelegramChatId
                : provider.contextProxy.getValue("notificationTelegramChatId");
            const dispatchResult = await dispatchFollowupNotification({ email, emailPassword, smsNumber, smsGateway, telegramBotToken, telegramChatId }, {
            ...
            if (!emailSent && !smsSent && !telegramSent) {
                await vscode.window.showErrorMessage(`Test notification failed: ${errors.join("; ") || "unknown error"}`);
                break;
            }
            const details = [];
            if (emailSent) {
        ...
        case "remoteControlEnabled":
            try {
                ...
                provider.log(`CloudService#updateUserSettings failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            try {
                await provider.remoteControlEnabled(message.bool ?? false);
            }
            catch (error) {
                provider.log(`ClineProvider#remoteControlEnabled failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            await provider.postStateToWebview();
            break;
        ...
        case "startRealtimeSession": {
            ...
            const requestValues = (message.values ?? {});
            ...
            try {
                ...
                provider.log(`[*Realtime] session ${session.sessionId} issued (model=${session.model}, expires=${new Date(session.expiresAt).toISOString()})`);
        ...
        case "stopRealtimeSession": {
            ...
            provider.log(`[*Realtime] stopRealtimeSession request${active ? ` (session=${active.sessionId})` : " (no active session)"}`);
        ...
        case "terminalOutputLineLimit":
            ...
            if (typeof lineLimit === "number" && lineLimit > 0) {
                ...
                vscode.window.showErrorMessage(t("common:errors.invalid_line_limit") || "Terminal output line limit must be a positive number");
            }
            break;
        case "terminalOutputCharacterLimit":
            ...
            if (typeof charLimit === "number" && charLimit > 0) {
                ...
                vscode.window.showErrorMessage(t("common:errors.invalid_character_limit") ||
        ...
        case "updateSupportPrompt":
            try {
                ...
                provider.log(`Error update support prompt: ${JSON.stringify(error, Object.getOwnPropertyNames(error), 2)}`);
        ...
        case "updatePrompt":
            if (message.promptMode && message.customPrompt !== undefined) {
                ...
                if (TelemetryService.hasInstance()) {
                    ...
                    const changedSettings = Object.keys(newPrompt).filter((key) => JSON.stringify(oldPrompt[key]) !==
                        JSON.stringify(newPrompt[key]));
        ...
        case "language":
            ...
            await updateGlobalState("language", message.text);
            await provider.postStateToWebview();
            break;
        ...
        case "enhancePrompt":
            if (message.text) {
                try {
                    ...
                    const { apiConfiguration, customSupportPrompts, listApiConfigMeta = [], enhancementApiConfigId, includeTaskHistoryInEnhance, } = state;
                    const currentCline = provider.getCurrentTask();
        ...
        case "showFeedbackOptions": {
            ...
            const answer = await vscode.window.showInformationMessage(t("common:feedback.description"), { modal: true }, githubIssuesText, discordText, customerSupport);
            ...
            if (message.text && message.apiConfiguration) {
                ...
                try {
                    ...
                    if (currentConfig.kilocodeOrganizationId !== message.apiConfiguration.kilocodeOrganizationId) {
                        ...
                        provider.postMessageToWebview({
                            type: "routerModels",
                            routerModels: { "kilocode-openrouter": models },
                        });
                    }
                }
                catch (error) {
        ...
        case "editMessageConfirm":
            if (message.messageTs && message.text) {
                await handleEditMessageConfirm(message.messageTs, message.text, message.restoreCheckpoint, message.images);
            }
            break;
        ...
        case "updateMcpTimeout":
            if (message.serverName && typeof message.timeout === "number") {
                try {
                    await provider
                        .getMcpHub()
                        ?.updateServerTimeout(message.serverName, message.timeout, message.source);
                }
                catch (error) {
            ...
            if (message.modeConfig) {
                ...
                if (TelemetryService.hasInstance()) {
                    if (isNewMode) {
                        ...
                        TelemetryService.instance.captureCustomModeCreated(message.modeConfig.slug, message.modeConfig.name);
                    }
                    else {
                        // Determine which setting was changed by comparing objects
                        const existingMode = existingModes.find((mode) => mode.slug === message.modeConfig?.slug);
                        const changedSettings = existingMode
                            ? Object.keys(message.modeConfig).filter((key) => JSON.stringify(existingMode[key]) !==
                                JSON.stringify(message.modeConfig[key]))
                            : [];
                        if (changedSettings.length > 0) {
                            TelemetryService.instance.captureModeSettingChanged(changedSettings[0]);
                        }
            ...
            if (message.slug) {
                ...
                if (rulesFolderExists) {
                    try {
                        ...
                        vscode.window.showErrorMessage(t("common:errors.delete_rules_folder_failed", {
                            rulesFolderPath,
                    ...
                    if (result.success && result.yaml) {
                        ...
                        if (lastExportPath) {
                            ...
                            if (workspaceFolders && workspaceFolders.length > 0) {
                                defaultUri = vscode.Uri.file(path.join(workspaceFolders[0].uri.fsPath, `${message.slug}-export.yaml`));
                            }
                            else {
        ...
        case "fetchProfileDataRequest":
            try {
                ...
                const organizationExists = (response.data.organizations ?? []).some(({ id }) => id === apiConfiguration?.kilocodeOrganizationId);
        ...
        case "fetchBalanceDataRequest": // New handler
            try {
                ...
                const errorMessage = error.response?.data?.message || error.message || "Failed to fetch balance data from backend.";
        ...
        case "shopBuyCredits": // New handler
            try {
                ...
                const response = await axios.post(`${baseUrl}/payments/topup?origin=extension&source=${source}&amount=${credits}`, {}, {
                    headers: {
                        Authorization: `Bearer ${kilocodeToken}`,
                        "Content-Type": "application/json",
                    },
        ...
        case "toggleWorkflow": {
            if (message.workflowPath && typeof message.enabled === "boolean" && typeof message.isGlobal === "boolean") {
                await toggleWorkflow(message.workflowPath, message.enabled, message.isGlobal, provider.contextProxy, provider.context);
        ...
        case "toggleRule": {
            if (message.rulePath && typeof message.enabled === "boolean" && typeof message.isGlobal === "boolean") {
                await toggleRule(message.rulePath, message.enabled, message.isGlobal, provider.contextProxy, provider.context);
                await provider.postRulesDataToWebview();
            }
            break;
        }
        ...
        case "telemetrySetting": {
            const telemetrySetting = message.text;
        ...
        case "installMarketplaceItem": {
            if (marketplaceManager && message.mpItem && message.mpInstallOptions) {
                try {
                    const configFilePath = await marketplaceManager.installMarketplaceItem(message.mpItem, message.mpInstallOptions);
                    await provider.postStateToWebview();
        ...
        case "removeInstalledMarketplaceItem": {
            if (marketplaceManager && message.mpItem && message.mpInstallOptions) {
                try {
                    ...
                    vscode.window.showErrorMessage(`Failed to remove marketplace item: ${error instanceof Error ? error.message : String(error)}`);
                    // Send error message to webview
                    provider.postMessageToWebview({
                        type: "marketplaceRemoveResult",
                        success: false,
        ...
        case "installMarketplaceItemWithParameters": {
            if (marketplaceManager && message.payload && "item" in message.payload && "parameters" in message.payload) {
                try {
                    ...
                    vscode.window.showErrorMessage(`Failed to install marketplace item: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            break;
        }
            ...
            try {
                ...
                try {
                    if (provider.context) {
                        ...
                        const entries = sopUtils.collectEnabledWorkflowSops(workflowState.localWorkflowToggles, workflowState.globalWorkflowToggles);
                    ...
                    provider.log(`Error fetching workflow SOPs: ${JSON.stringify(workflowError, Object.getOwnPropertyNames(workflowError ?? {}), 2)}`);
                }
        ...
        case "createCommand": {
            try {
                ...
                try {
                    ...
                    provider.log(`Error refreshing workflow SOPs: ${JSON.stringify(workflowError, Object.getOwnPropertyNames(workflowError ?? {}), 2)}`);
                }
                const documentCommandList = commands.map((command) => ({
                    name: command.name,
                    displayName: command.name,
