...
import { openRouterDefaultModelId, ORGANIZATION_ALLOW_ALL, } from "@roo-code/types"; // kilocode_change: openRouterDefaultModelId
...
vi.mocked(await import("../../prompts/sections/custom-instructions")).addCustomInstructions =
...
vi.mock("../../task/Task", () => ({
    Task: vi
        .fn()
        .mockImplementation((_provider, _apiConfiguration, _customInstructions, _diffEnabled, _fuzzyMatchThreshold, _task, taskId) => ({
        api: undefined,
        abortTask: vi.fn(),
...
describe("ClineProvider", () => {
    ...
    beforeEach(() => {
        ...
        mockContext = {
            ...
            globalState: {
                ...
                update: vi
                    .fn()
                    .mockImplementation((key, value) => (globalState[key] = value)),
    ...
    test("resolveWebviewView sets up webview correctly in development mode even if local server is not running", async () => {
        provider = new ClineProvider({ ...mockContext, extensionMode: vscode.ExtensionMode.Development }, mockOutputChannel, "sidebar", new ContextProxy(mockContext));
    ...
    test("autoCondenseContextPercent defaults to 100", async () => {
        ...
        ;
        mockContext.globalState.get.mockImplementation((key) => key === "autoCondenseContextPercent" ? undefined : null);
        const state = await provider.getState();
    ...
    describe("createTaskWithHistoryItem mode validation", () => {
        test("validates and falls back to default mode when restored mode no longer exists", async () => {
            ...
            expect(logSpy).toHaveBeenCalledWith("Mode 'non-existent-mode' from history no longer exists. Falling back to default mode 'code'.");
        ...
        test("continues with task restoration even if mode config loading fails", async () => {
            ...
            expect(logSpy).toHaveBeenCalledWith(expect.stringContaining("Failed to restore API configuration for mode 'code'"));
        });
    });
    ...
    describe("Clover session cross-context synchronization", () => {
        ...
        beforeEach(async () => {
            ...
            mockContext.globalState.update = vi.fn().mockImplementation((key, value) => {
            ...
            sidebarProvider.view = sidebarWebview;
            tabProvider.view = tabWebview;
            cloverService = new CloverSessionService(mockContext, () => undefined);
            cloverService.useLocalFallback = true;
            await cloverService.ensureLocalSessionsLoaded();
        ...
        afterEach(async () => {
            ...
            ;
            ClineProvider.globalCloverSessionService = undefined;
        ...
        test("propagates new Clover sessions across active providers", async () => {
            ...
            expect(tabState.cloverMessages.map((message) => message.id)).toEqual(sidebarState.cloverMessages.map((message) => message.id));
            const postMessageMock = tabWebview.webview.postMessage;
            const tabMessages = postMessageMock.mock.calls.map(([message]) => message);
            expect(tabMessages.some((message) => message?.type === "outerGateState")).toBe(true);
        });
        test("shares appended Clover messages without duplication", async () => {
            ...
            expect(new Set(tabState.cloverMessages.map((message) => message.id)).size).toBe(tabState.cloverMessages.length);
            expect(tabState.cloverMessages.map((message) => message.text)).toEqual(sidebarState.cloverMessages.map((message) => message.text));
            expect(tabState.cloverSessions.entries[0].messageCount).toBe(sidebarState.cloverSessions.entries[0].messageCount);
        });
        test("persists Clover sessions across provider reload", async () => {
            ...
            reloadService.useLocalFallback = true;
            await reloadService.ensureLocalSessionsLoaded();
            sidebarProvider.attachCloverSessionService(reloadService);
            tabProvider.attachCloverSessionService(reloadService);
            await sidebarProvider.refreshCloverSessions({ append: false, syncActiveSession: true });
            ...
            expect(new Set(reloadedSidebarState.cloverMessages.map((message) => message.id)).size).toBe(reloadedSidebarState.cloverMessages.length);
            expect(reloadedTabState.cloverMessages.map((message) => message.text)).toEqual(reloadedSidebarState.cloverMessages.map((message) => message.text));
        });
    });
});
...
describe("ClineProvider - Router Models", () => {
    ...
    test("skips LiteLLM when neither config nor message values are provided", async () => {
        ...
        expect(getModels).not.toHaveBeenCalledWith(expect.objectContaining({
            provider: "litellm",
        }));
...
describe("ClineProvider - Comprehensive Edit/Delete Edge Cases", () => {
    ...
    describe("Edit Messages with Images and Attachments", () => {
        ...
        test("handles editing messages with file attachments", async () => {
            ...
            expect(mockCline.handleWebviewAskResponse).toHaveBeenCalledWith("messageResponse", "Edited message with file attachment", undefined);
        });
    });
    ...
    describe("Edit Permissions and Authorization", () => {
        ...
        describe("Resource Cleanup During Failed Operations", () => {
            ...
            test("validates proper cleanup during failed delete operations", async () => {
                ...
                // Verify cleanup was attempted before failure
                expect(cleanupSpy).toHaveBeenCalled();
                expect(vscode.window.showErrorMessage).toHaveBeenCalledWith("Error deleting message: Delete operation failed");
            });
        });
        describe("Large Message Payloads", () => {
            ...
            test("handles editing messages with large text content", async () => {
                ...
                expect(mockCline.overwriteClineMessages).toHaveBeenCalled();
                expect(mockCline.handleWebviewAskResponse).toHaveBeenCalledWith("messageResponse", largeEditedContent, undefined);
            });
...
//# sourceMappingURL=ClineProvider.spec.js.map