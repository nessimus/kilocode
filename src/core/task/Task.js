...
import { RooCodeEventName, TelemetryEventName, TaskStatus, DEFAULT_CONSECUTIVE_MISTAKE_LIMIT, getApiProtocol, getModelId, isIdleAsk, isInteractiveAsk, isResumableAsk, } from "@roo-code/types";
...
import { getCheckpointService, checkpointSave, checkpointRestore, checkpointDiff, } from "../checkpoints";
...
export class Task extends EventEmitter {
    ...
    taskId;
    taskIsFavorited; // kilocode_change
    rootTaskId;
    parentTaskId;
    childTaskId;
    instanceId;
    ...
    isInitialized = false;
    isPaused = false;
    pausedModeSlug = defaultModeSlug;
    pauseInterval;
    // API
    apiConfiguration;
    api;
    ...
    constructor({ context, // kilocode_change
    provider, apiConfiguration, enableDiff = false, enableCheckpoints = true, enableBridge = false, fuzzyMatchThreshold = 1.0, consecutiveMistakeLimit = DEFAULT_CONSECUTIVE_MISTAKE_LIMIT, task, images, historyItem, startTask = true, rootTask, parentTask, taskNumber = -1, onCreated, initialTodos, workspacePath, }) {
        super();
        ...
        if (this.diffEnabled) {
            ...
            provider.getState().then((state) => {
                const isMultiFileApplyDiffEnabled = experiments.isEnabled(state.experiments ?? {}, EXPERIMENT_IDS.MULTI_FILE_APPLY_DIFF);
    ...
    async ask(type, text, partial, progressStatus, isProtected) {
        ...
        let statusMutationTimeouts = [];
        if (isStatusMutable) {
            console.log(`Task#ask will block -> type: ${type}`);
            if (isInteractiveAsk(type)) {
                statusMutationTimeouts.push(setTimeout(() => {
    ...
    setMessageResponse(text, images) {
    ...
    handleWebviewAskResponse(askResponse, text, images) {
        ...
        if (askResponse === "messageResponse" || askResponse === "yesButtonClicked") {
            ...
            const lastFollowUpIndex = findLastIndex(this.clineMessages, (msg) => msg.type === "ask" && msg.ask === "followup" && !msg.isAnswered);
    ...
    async condenseContext() {
        ...
        const { messages, summary, cost, newContextTokens = 0, error, } = await summarizeConversation(this.apiConversationHistory, this.api, // Main API handler (fallback)
    ...
    async sayAndCreateMissingParamError(toolName, paramName, relPath) {
        await this.say("error", `Kilo Code tried to use ${toolName}${relPath ? ` for '${relPath.toPosix()}'` : ""} without value for required parameter '${paramName}'. Retrying...`);
    ...
    async resumeTaskFromHistory() {
        ...
        const gpt5Messages = modifiedClineMessages.filter((m) => m.type === "say" &&
            m.say === "text" &&
            !!m.metadata?.gpt5?.previous_response_id);
    ...
    async recursivelyMakeClineRequests(userContent, includeFileDetails = false) {
        ...
        while (stack.length > 0) {
            ...
            await this.say("api_req_started", JSON.stringify({
                request: currentUserContent.map((block) => formatContentBlockToMarkdown(block)).join("\n\n") +
                    "\n\nLoading...",
                systemPrompt,
                apiProtocol,
            }));
            ...
            if (needsRulesFileCheck) {
                await this.say("error", "Issue with processing the /newrule command. Double check that, if '.kilocode/rules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory");
            }
            // kilocode_change end
            ...
            try {
                ...
                try {
                    ...
                    const drainStreamInBackgroundToFindAllUsage = async (apiReqIndex) => {
                        ...
                        // kilocode_change start
                        const refreshApiReqMsg = async (messageIndex) => {
                        ...
                        const captureUsageData = async (tokens, messageIndex = apiReqIndex) => {
                            if (tokens.input > 0 ||
                                ...
                                TelemetryService.instance.captureLlmCompletion(this.taskId, {
                                    ...
                                    cacheReadTokens: tokens.cacheRead,
                                    cost: tokens.total ??
                                        calculateApiCostAnthropic(this.api.getModel().info, tokens.input, tokens.output, tokens.cacheWrite, tokens.cacheRead),
                ...
                if (assistantMessage.length > 0) {
                    ...
                    // an error.
                    await this.say("error", t("kilocode:task.noAssistantMessages"));
                    // kilocode_change start
    ...
    async loadContext(userContent, includeFileDetails = false) {
        ...
        const processUserContent = async () => {
            ...
            return await Promise.all(userContent.map(async (block) => {
                if (block.type === "text") {
                    ...
                    if (block.text.includes("<feedback>") ||
                        ...
                        const parsedText = await parseMentions(block.text, this.cwd, this.urlContentFetcher, this.fileContextTracker);
                        // when parsing slash commands, we still want to allow the user to provide their desired context
                        const { processedText, needsRulesFileCheck: needsCheck } = await parseKiloSlashCommands(parsedText, localWorkflowToggles, globalWorkflowToggles);
                        if (needsCheck) {
    ...
    // kilocode_change end
    /*private kilocode_change*/ async getSystemPrompt() {
        ...
        return await (async () => {
            ...
            return SYSTEM_PROMPT(provider.context, this.cwd, 
            // kilocode_change: supports images => supports browser
            (this.api.getModel().info.supportsImages ?? false) && (browserToolEnabled ?? true), mcpHub, this.diffStrategy, browserViewportSize, mode, customModePrompts, customModes, customInstructions, this.diffEnabled, experiments, enableMcpServerCreation, language, rooIgnoreInstructions, maxReadFileLine !== -1, {
            ...
            this.api.getModel().id, await provider.getState());
        })();
    }
    ...
    async *attemptApiRequest(retryAttempt = 0) {
        ...
        let cleanConversationHistory = maybeRemoveImageBlocks(messagesSinceLastSummary, this.api).map(({ role, content }) => ({ role, content }));
        ...
        try {
            ...
            if (modelId && modelId.startsWith("gpt-5") && !this.skipPrevResponseIdOnce) {
                ...
                const idx = findLastIndex(this.clineMessages, (m) => m.type === "say" &&
                    m.say === "text" &&
                    !!m.metadata?.gpt5?.previous_response_id);
                if (idx !== -1) {
        ...
        try {
            ...
            if (apiConfiguration?.apiProvider === "kilocode" && isAnyRecognizedKiloCodeError(error)) {
                const { response } = await (isPaymentRequiredError(error)
                    ? this.ask("payment_required_prompt", JSON.stringify({
                        ...
                        balance: error.balance ?? "0.00",
                        buyCreditsUrl: error.buyCreditsUrl ?? "https://kilocode.ai/profile",
                    }))
            ...
            if (autoApprovalEnabled && alwaysApproveResubmit) {
                ...
                if (error.status === 429) {
                    const geminiRetryDetails = error.errorDetails?.find((detail) => detail["@type"] === "type.googleapis.com/google.rpc.RetryInfo");
     ...
     */
    async persistGpt5Metadata(reasoningMessage) {
        try {
            const modelId = this.api.getModel().id;
            ...
            const idx = findLastIndex(this.clineMessages, (m) => m.type === "say" && m.say === "text" && m.partial !== true);
            if (idx !== -1) {
