...
const detectExplicitMentions = (
	...
	for (const match of matches) {
		...
		const candidate =
			agentLookup.byName.get(normalized) ?? agentLookup.tokens.get(normalized) ?? undefined
		if (candidate) {
			found.push({
				id: candidate.id,
...
const detectBackgroundMonitors = (
	agents: ConversationAgent[],
	selected: Set<string>,
): OrchestratorAgent[] => {
	return agents
...
export class ConversationOrchestrator {
	...
	analyze({ text, agents, activeAgentId, holdState, timestamp }: AnalyzeMessageInput): OrchestratorAnalysis {
		...
		const routedAgents: OrchestratorAgent[] = [...explicitMentions, ...roleMatches]
		...
		if (explicitMentions.length) {
			rationaleSegments.push(
				`Directed mention${explicitMentions.length > 1 ? "s" : ""}: ${explicitMentions
		...
		if (roleMatches.length) {
			rationaleSegments.push(
				`Topic match: ${roleMatches.map((agent) => agent.label ?? agent.id).join(", ")}`,
			)
		}
