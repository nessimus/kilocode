import {
	...
	OrchestratorAgent,
	OrchestratorHoldMode,
	OrchestratorIntent,
	OrchestratorSuppressedAgent,
} from "@roo-code/types"
...
export interface AnalyzeMessageInput {
	...
	holdState?: ConversationHoldState
	timestamp?: number
}
...
type SingleSpeakerDirectiveReason = "document_block" | "direct_speech"

const detectSingleSpeakerDirective = (lowerText: string): SingleSpeakerDirectiveReason | undefined => {
...
const normalizeWhitespacePattern = (value: string) => escapeRegex(value).replace(/\s+/g, "\\s+")

const buildAgentAliases = (agent: ConversationAgent): string[] => {
...
const buildAgentNameAliases = (agent: ConversationAgent): string[] => {
	const aliases = new Set<string>()
	const normalizedName = normalize(agent.name)
	if (!normalizedName) {
		return []
...
const matchesExclusiveCue = (text: string, alias: string): boolean => {
	if (!alias) {
		return false
	}
...
const matchesExclusionCue = (text: string, alias: string): boolean => {
	if (!alias) {
		return false
	}
	const pattern = normalizeWhitespacePattern(alias)
...
const buildAgentLookup = (agents: ConversationAgent[]) => {
	const byId = new Map<string, ConversationAgent>()
	const byName = new Map<string, ConversationAgent>()
	const tokens = new Map<string, ConversationAgent>()
...
const detectExplicitMentions = (
	text: string,
	agentLookup: ReturnType<typeof buildAgentLookup>,
): OrchestratorAgent[] => {
	const matches = text.matchAll(MENTION_REGEX)
	const found: OrchestratorAgent[] = []
...
const detectDirectNameMentions = (
	text: string,
	agents: ConversationAgent[],
	excludedIds: Set<string>,
): OrchestratorAgent[] => {
	const results: OrchestratorAgent[] = []
	agents.forEach((agent) => {
...
const detectRoleMatches = (
	text: string,
	agents: ConversationAgent[],
	alreadySelected: Set<string>,
): OrchestratorAgent[] => {
	const lowerText = text.toLowerCase()
	const matches: OrchestratorAgent[] = []
...
const detectBackgroundMonitors = (agents: ConversationAgent[], selected: Set<string>): OrchestratorAgent[] => {
	return agents
		.filter((agent) => !selected.has(agent.id))
		.map<OrchestratorAgent>((agent) => ({
			id: agent.id,
			label: agent.name,
			role: agent.role,
...
const parseIntent = (
	...
	const lowerText = text.toLowerCase()
	const mentions: OrchestratorIntent["mentions"] = []
	...
	const impliedRoles: string[] = []
	const exclusiveAgentIds = new Set<string>()
	...
	const urgency = URGENCY_KEYWORDS.some((keyword) => lowerText.includes(keyword)) ? "high" : undefined
	...
	return {
		...
		exclusiveAgentIds: exclusiveList.length ? exclusiveList : undefined,
		excludedAgentIds: excludedList.length ? excludedList : undefined,
		multiSpeaker,
	}
}
...
const buildSuppressedAgents = (
	agents: ConversationAgent[],
	selectedIds: Set<string>,
): OrchestratorSuppressedAgent[] => {
	return agents
		.filter((agent) => !selectedIds.has(agent.id))
		.map<OrchestratorSuppressedAgent>((agent) => ({
			id: agent.id,
			label: agent.name,
...
export class ConversationOrchestrator {
	private fairnessCounters = new Map<string, number>()
	...
	registerResponse(agentIds: string[]): void {
		agentIds.forEach((id) => {
	...
	analyze({ text, agents, activeAgentId, holdState, timestamp }: AnalyzeMessageInput): OrchestratorAnalysis {
		...
		const routedAgents: OrchestratorAgent[] = prioritizedAgents.map(
			({ agent, isExclusive, isDirected, isRoleMatch, isActive }) => {
				...
				const tier: OrchestratorAgent["tier"] =
					isExclusive || isDirected ? "directed" : isRoleMatch ? "topic" : "rotation"
				return {
					id: agent.id,
					label: agent.name,
					role: agent.role,
		...
		const resolvedIntent: OrchestratorIntent = {
			...intent,
			exclusiveAgentIds: resolvedExclusiveIds.length ? resolvedExclusiveIds : undefined,
			excludedAgentIds: resolvedExcludedIds.length ? resolvedExcludedIds : undefined,
			multiSpeaker,
			directedAgentIds: directedAgentIds.length ? directedAgentIds : undefined,
			respondDirectly: respondDirectly || undefined,
			documentCreationBanned: documentCreationBanned || undefined,
		}
		...
		const holdMode: OrchestratorHoldMode | undefined = shouldHoldForStop
			? "manual_hold"
			: holdState?.mode === "manual_hold"
				? "manual_hold"
				: "ingest_hold"

		const hold: OrchestratorAnalysis["hold"] = {
			mode: holdMode,
		...
		const result: OrchestratorAnalysis = {
...
export const createManualHoldState = (
	initiatedBy: ConversationHoldState["initiatedBy"] = "user",
): ConversationHoldState => ({
	mode: "manual_hold",
	initiatedBy,
...
export const createIngestHoldState = (): ConversationHoldState => ({
	mode: "ingest_hold",
	initiatedBy: "system",
