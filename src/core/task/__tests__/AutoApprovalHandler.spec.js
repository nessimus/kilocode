...
describe("AutoApprovalHandler", () => {
    ...
    let mockState;
    const mockGetApiMetrics = getApiMetrics;
    beforeEach(() => {
    ...
    describe("checkAutoApprovalLimits", () => {
        ...
        it("should check request limit before cost limit", async () => {
            ...
            expect(mockAskForApproval).toHaveBeenCalledWith("auto_approval_max_req_reached", JSON.stringify({ count: 1, type: "requests" }));
    ...
    describe("request limit handling", () => {
        ...
        it("should ask for approval when limit is exceeded", async () => {
            ...
            mockAskForApproval.mockResolvedValue({ response: "yesButtonClicked" });
            const result = await handler.checkAutoApprovalLimits(mockState, messages, mockAskForApproval);
            expect(mockAskForApproval).toHaveBeenCalledWith("auto_approval_max_req_reached", JSON.stringify({ count: 3, type: "requests" }));
            expect(result.shouldProceed).toBe(true);
    ...
    describe("cost limit handling", () => {
        ...
        it("should ask for approval when cost limit is exceeded", async () => {
            ...
            mockAskForApproval.mockResolvedValue({ response: "yesButtonClicked" });
            const result = await handler.checkAutoApprovalLimits(mockState, messages, mockAskForApproval);
            expect(mockAskForApproval).toHaveBeenCalledWith("auto_approval_max_req_reached", JSON.stringify({ count: "5.00", type: "cost" }));
            expect(result.shouldProceed).toBe(true);
            expect(result.requiresApproval).toBe(true);
    ...
    describe("combined limits", () => {
        it("should handle both request and cost limits", async () => {
            ...
            mockAskForApproval.mockResolvedValue({ response: "yesButtonClicked" });
            const result = await handler.checkAutoApprovalLimits(mockState, messages, mockAskForApproval);
            expect(mockAskForApproval).toHaveBeenCalledWith("auto_approval_max_req_reached", JSON.stringify({ count: 2, type: "requests" }));
            expect(result.shouldProceed).toBe(true);
            expect(result.requiresApproval).toBe(true);
...
//# sourceMappingURL=AutoApprovalHandler.spec.js.map