...
describe("Cline", () => {
    ...
    beforeEach(() => {
        ...
        mockProvider = new ClineProvider(mockExtensionContext, mockOutputChannel, "sidebar", new ContextProxy(mockExtensionContext));
    ...
    describe("getEnvironmentDetails", () => {
        describe("API conversation handling", () => {
            ...
            it.skip("should not apply retry delay twice", async () => {
                ...
                const retryMessages = saySpy.mock.calls.filter((call) => call[0] === "api_req_retry_delayed" && call[1]?.includes("Retrying in"));
            ...
            describe("processUserContentMentions", () => {
                it("should process mentions in task and feedback tags", async () => {
                    ...
                    expect(processedContent[1].text).toContain("<task>Text with 'some/path' (see below for file content) in task tags</task>");
                    // Feedback tag content should be processed
                    const toolResult1 = processedContent[2];
                    const content1 = Array.isArray(toolResult1.content) ? toolResult1.content[0] : toolResult1.content;
                    expect(content1.text).toContain("processed:");
                    expect(content1.text).toContain("<feedback>Check 'some/path' (see below for file content)</feedback>");
        ...
        describe("Subtask Rate Limiting", () => {
            ...
            it("should update global timestamp even when no rate limiting is needed", async () => {
                ...
                const globalTimestamp = Task.lastGlobalApiRequestTime;
