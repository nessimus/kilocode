...
describe("summarizeConversation", () => {
    ...
    it("should not summarize when there are not enough messages", async () => {
        ...
        const result = await summarizeConversation(messages, mockApiHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS);
    ...
    it("should handle empty summary response and return error", async () => {
        ...
        maybeRemoveImageBlocks.mockImplementationOnce((messages) => {
    ...
    it("should correctly format the request to the API", async () => {
        ...
        expect(mockApiHandler.createMessage).toHaveBeenCalledWith(expect.stringContaining("Your task is to create a detailed summary of the conversation"), expect.any(Array));
        // Check that maybeRemoveImageBlocks was called with the correct messages
        const mockCallArgs = maybeRemoveImageBlocks.mock.calls[0][0];
    ...
    it("should calculate newContextTokens correctly with systemPrompt", async () => {
        ...
        const result = await summarizeConversation(messages, mockApiHandler, systemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS);
    ...
    it("should return error when new context tokens >= previous context tokens", async () => {
        ...
        const result = await summarizeConversation(messages, mockApiHandler, defaultSystemPrompt, taskId, prevContextTokens);
    ...
    it("should return error when both condensing and main API handlers are invalid", async () => {
        ...
        const result = await summarizeConversation(messages, invalidMainHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS, false, undefined, invalidCondensingHandler);
        ...
        expect(mockError).toHaveBeenCalledWith(expect.stringContaining("Main API handler is also invalid for condensing"));
...
describe("summarizeConversation with custom settings", () => {
    ...
    beforeEach(() => {
        // Reset mocks
        vi.clearAllMocks();
        TelemetryService.instance.captureContextCondensed.mockClear();
    ...
    it("should use custom prompt when provided", async () => {
        ...
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS, false, customPrompt);
    ...
    it("should use default systemPrompt when custom prompt is empty or not provided", async () => {
        ...
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS, false, "  ");
        ...
        vi.clearAllMocks();
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS, false, undefined);
    ...
    it("should use condensingApiHandler when provided and valid", async () => {
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS, false, undefined, mockCondensingApiHandler);
    ...
    it("should fall back to mainApiHandler if condensingApiHandler is not provided", async () => {
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS, false, undefined, undefined);
    ...
    it("should fall back to mainApiHandler if condensingApiHandler is invalid", async () => {
        ...
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS, false, undefined, invalidHandler);
        // Verify the main handler was used as fallback
        expect(mockMainApiHandler.createMessage.mock.calls.length).toBe(1);
        // Verify warning was logged
        expect(mockWarn).toHaveBeenCalledWith(expect.stringContaining("Chosen API handler for condensing does not support message creation"));
    ...
    it.skip("should capture telemetry when using custom prompt", async () => {
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS, false, "Custom prompt");
        // Verify telemetry was called with custom prompt flag
        expect(TelemetryService.instance.captureContextCondensed).toHaveBeenCalledWith(taskId, false, true, // usedCustomPrompt
        false);
    });
    /**
    ...
    it.skip("should capture telemetry when using custom API handler", async () => {
        ...
        expect(TelemetryService.instance.captureContextCondensed).toHaveBeenCalledWith(taskId, false, false, // usedCustomPrompt
        true);
    });
    /**
    ...
    it.skip("should capture telemetry when using both custom prompt and API handler", async () => {
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS, true, // isAutomaticTrigger
        "Custom prompt", mockCondensingApiHandler);
        // Verify telemetry was called with both flags
        expect(TelemetryService.instance.captureContextCondensed).toHaveBeenCalledWith(taskId, true, // isAutomaticTrigger
