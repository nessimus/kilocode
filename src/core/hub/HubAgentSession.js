...
export class HubAgentSession {
    participant;
    provider;
    constructor(participant, provider) {
    ...
    buildSystemPrompt(context, state) {
        ...
        if (persona?.instructions) {
            personaSections.push(`Role Instructions for ${this.participant.displayName} (follow carefully):\n${persona.instructions}`);
        }
        if (persona?.role) {
        ...
        if (workplaceState) {
            const activeCompany = workplaceState.companies.find((company) => company.id === workplaceState.activeCompanyId) ??
            ...
            if (activeCompany) {
                ...
                if (activeCompany.actionItems?.length) {
                    profileLines.push(`Active Initiatives: ${activeCompany.actionItems.length} tracked objectives/projects.`);
                }
                ...
                if (persona?.employeeId) {
                    ...
                    if (employee) {
                        ...
                        if (employee.role)
                            employeeLines.push(`- Role: ${employee.role}`);
                        if (employee.personality)
                            employeeLines.push(`- Persona Traits: ${employee.personality}`);
                        if (employee.mbtiType)
                            employeeLines.push(`- MBTI: ${employee.mbtiType}`);
    ...
    toMessageParams(context) {
        ...
        return context.messages.map((message) => {
            ...
            const role = participant?.type === "agent" ? "assistant" : "user";
    ...
    async respond(context, onUpdate) {
...
//# sourceMappingURL=HubAgentSession.js.map