...
describe("ProviderSettingsManager", () => {
    ...
    describe("initialize", () => {
        ...
        it("should throw error if secrets storage fails", async () => {
            ...
            await expect(providerSettingsManager.initialize()).rejects.toThrow("Failed to initialize config: Error: Failed to read provider profiles from secrets: Error: Storage failed");
        });
    });
    describe("ListConfig", () => {
        ...
        it("should throw error if reading from secrets fails", async () => {
            ...
            await expect(providerSettingsManager.listConfig()).rejects.toThrow("Failed to list configs: Error: Failed to read provider profiles from secrets: Error: Read failed");
        });
    });
    describe("SaveConfig", () => {
        ...
        it("should update existing config", async () => {
            ...
            const storedConfig = JSON.parse(mockSecrets.store.mock.calls[mockSecrets.store.mock.calls.length - 1][1]);
            expect(mockSecrets.store.mock.calls[mockSecrets.store.mock.calls.length - 1][0]).toEqual("roo_cline_config_api_config");
            expect(storedConfig).toEqual(expectedConfig);
        });
        it("should throw error if secrets storage fails", async () => {
            mockSecrets.get.mockResolvedValue(JSON.stringify({
            ...
            await expect(providerSettingsManager.saveConfig("test", {})).rejects.toThrow("Failed to save config: Error: Failed to write provider profiles to secrets: Error: Storage failed");
        });
    });
    describe("DeleteConfig", () => {
        ...
        it("should throw error when trying to delete non-existent config", async () => {
            ...
            await expect(providerSettingsManager.deleteConfig("nonexistent")).rejects.toThrow("Config 'nonexistent' not found");
        });
        it("should throw error when trying to delete last remaining config", async () => {
            ...
            await expect(providerSettingsManager.deleteConfig("default")).rejects.toThrow("Failed to delete config: Error: Cannot delete the last remaining configuration");
        });
    });
    describe("LoadConfig", () => {
        ...
        it("should throw error when config does not exist", async () => {
            ...
            await expect(providerSettingsManager.activateProfile({ name: "nonexistent" })).rejects.toThrow("Config with name 'nonexistent' not found");
        });
        it("should throw error if secrets storage fails", async () => {
            mockSecrets.get.mockResolvedValue(JSON.stringify({
                currentApiConfigName: "default",
            ...
            mockSecrets.store.mockRejectedValue(new Error("Storage failed"));
            await expect(providerSettingsManager.activateProfile({ name: "test" })).rejects.toThrow("Failed to activate profile: Failed to write provider profiles to secrets: Error: Storage failed");
        });
    ...
    describe("HasConfig", () => {
        ...
        it("should return false for non-existent config", async () => {
            mockSecrets.get.mockResolvedValue(JSON.stringify({ currentApiConfigName: "default", apiConfigs: { default: {} } }));
            const hasConfig = await providerSettingsManager.hasConfig("nonexistent");
            expect(hasConfig).toBe(false);
        });
        it("should throw error if secrets storage fails", async () => {
            mockSecrets.get.mockRejectedValue(new Error("Storage failed"));
            await expect(providerSettingsManager.hasConfig("test")).rejects.toThrow("Failed to check config existence: Error: Failed to read provider profiles from secrets: Error: Storage failed");
        });
    });
    describe("syncCloudProfiles", () => {
        ...
        it("should delete cloud profiles not in the new cloud profiles", async () => {
            const existingConfig = {
                ...
                apiConfigs: {
                    default: { id: "default-id" },
                    "cloud-profile-1": { id: "cloud-id-1", apiProvider: "anthropic" },
                    "cloud-profile-2": { id: "cloud-id-2", apiProvider: "openai" },
                },
        ...
        it("should rename existing non-cloud profile when cloud profile has same name", async () => {
            const existingConfig = {
                ...
                apiConfigs: {
                    default: { id: "default-id" },
                    "conflict-name": { id: "local-id", apiProvider: "openai" },
                },
                cloudProfileIds: [],
            };
            mockSecrets.get.mockResolvedValue(JSON.stringify(existingConfig));
        ...
        it("should handle multiple naming conflicts with incremental suffixes", async () => {
            const existingConfig = {
                ...
                apiConfigs: {
                    default: { id: "default-id" },
                    "conflict-name": { id: "local-id-1", apiProvider: "openai" },
                    "conflict-name_local": { id: "local-id-2", apiProvider: "vertex" },
                },
                cloudProfileIds: [],
            };
            mockSecrets.get.mockResolvedValue(JSON.stringify(existingConfig));
        ...
        it("should handle complex sync scenario with multiple operations", async () => {
            const existingConfig = {
                ...
                apiConfigs: {
                    default: { id: "default-id" },
                    "keep-cloud": { id: "cloud-id-1", apiProvider: "anthropic", apiKey: "secret1" },
                    "delete-cloud": { id: "cloud-id-2", apiProvider: "openai" },
                    "rename-me": { id: "local-id", apiProvider: "vertex" },
                },
                cloudProfileIds: ["cloud-id-1", "cloud-id-2"],
            };
            mockSecrets.get.mockResolvedValue(JSON.stringify(existingConfig));
        ...
        it("should throw error if secrets storage fails", async () => {
            ...
            mockSecrets.store.mockRejectedValue(new Error("Storage failed"));
            await expect(providerSettingsManager.syncCloudProfiles({})).rejects.toThrow("Failed to sync cloud profiles: Error: Failed to write provider profiles to secrets: Error: Storage failed");
        });
        ...
        it("should track active profile changes when active profile is deleted", async () => {
            const existingConfig = {
                currentApiConfigName: "active-profile",
                apiConfigs: {
                    "active-profile": { id: "active-id", apiProvider: "anthropic" },
                    "backup-profile": { id: "backup-id", apiProvider: "openai" },
                },
                cloudProfileIds: ["active-id"],
            };
            mockSecrets.get.mockResolvedValue(JSON.stringify(existingConfig));
        ...
        it("should create default profile when all profiles are deleted", async () => {
            const existingConfig = {
                ...
                apiConfigs: {
                    "only-profile": { id: "only-id", apiProvider: "anthropic" },
                },
        ...
        it("should not mark active profile as changed when it's not affected", async () => {
            const existingConfig = {
                ...
                apiConfigs: {
                    "local-profile": { id: "local-id", apiProvider: "anthropic" },
                    "cloud-profile": { id: "cloud-id", apiProvider: "openai" },
                },
...
//# sourceMappingURL=ProviderSettingsManager.spec.js.map