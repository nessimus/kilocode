...
describe("importExport", () => {
    ...
    describe("importSettings", () => {
        ...
        it("should import settings from provided file path without showing dialog", async () => {
            ...
            fs.readFile.mockResolvedValue(mockFileContent);
            fs.access.mockResolvedValue(undefined); // File exists and is readable
            const previousProviderProfiles = {
                currentApiConfigName: "default",
                apiConfigs: { default: { apiProvider: "anthropic", id: "default-id" } },
            };
    ...
    describe("exportSettings", () => {
        ...
        it("should handle errors during the export process", async () => {
            ...
            mockContextProxy.export.mockResolvedValue({ mode: "code" });
            safeWriteJson.mockRejectedValueOnce(new Error("Safe write error"));
            await exportSettings({
                providerSettingsManager: mockProviderSettingsManager,
                contextProxy: mockContextProxy,
            });
        ...
        it("should use the correct default save location", async () => {
            ...
            expect(vscode.Uri.file).toHaveBeenCalledWith(path.join("/mock/home", "Documents", "kilo-code-settings.json"));
        });
        describe("codebase indexing export", () => {
            ...
            it("should export model dimension for OpenAI Compatible provider", async () => {
                ...
                const exportedData = safeWriteJson.mock.calls[0][1];
                ...
                expect(exportedData.globalSettings.codebaseIndexConfig.codebaseIndexOpenAiCompatibleBaseUrl).toBe("https://api.example.com/v1");
            });
            it("should not mix settings between different providers", async () => {
                ...
                const exportedData = safeWriteJson.mock.calls[0][1];
                // Settings are now exported as-is from codebaseIndexConfig
                expect(exportedData.globalSettings.codebaseIndexConfig.codebaseIndexOpenAiCompatibleBaseUrl).toBe("https://openai-compatible.example.com/v1");
                ...
                expect(exportedData.globalSettings.codebaseIndexConfig.codebaseIndexEmbedderBaseUrl).toBe("http://localhost:11434");
            });
        ...
        describe("import with OpenAI Compatible codebase indexing settings", () => {
            it("should properly import OpenAI Compatible settings in codebaseIndexConfig", async () => {
                ...
                expect(mockContextProxy.setValues).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it("should preserve model dimension exactly in export/import roundtrip", async () => {
            ...
            fs.readFile.mockResolvedValue(exportedFileContent);
            ...
            expect(importedGlobalSettings.codebaseIndexConfig?.codebaseIndexEmbedderModelDimension).toBe(testModelDimension);
            expect(importedGlobalSettings.codebaseIndexConfig?.codebaseIndexOpenAiCompatibleBaseUrl).toBe("https://api.example.com/v1");
        ...
        it("should handle provider mismatch during import - BUG REPRODUCTION", async () => {
            ...
            vscode.window.showOpenDialog.mockResolvedValue([{ fsPath: "/mock/path/settings.json" }]);
            fs.readFile.mockResolvedValue(JSON.stringify(exportedSettings));
        ...
        it("should NOT copy OpenAI Compatible settings to provider profiles - FIXED BEHAVIOR", async () => {
            ...
            const importedGlobalSettings = mockContextProxy.setValues.mock.calls[0][0];
            expect(importedGlobalSettings.codebaseIndexConfig?.codebaseIndexOpenAiCompatibleBaseUrl).toBe("https://new-url.example.com/v1");
        ...
        it("should keep OpenAI Compatible settings in global state only - FIXED BEHAVIOR", async () => {
            ...
            // Verify OpenAI Compatible settings are imported to global settings
            const importedGlobalSettings = mockContextProxy.setValues.mock.calls[0][0];
            expect(importedGlobalSettings.codebaseIndexConfig?.codebaseIndexOpenAiCompatibleBaseUrl).toBe("https://updated.example.com/v1");
            expect(importedGlobalSettings.codebaseIndexConfig?.codebaseIndexOpenAiCompatibleModelDimension).toBe(1536);
        ...
        it("should export OpenAI Compatible settings from global state when provider is openai-compatible", async () => {
            ...
            expect(exportedData.globalSettings.codebaseIndexConfig.codebaseIndexOpenAiCompatibleModelDimension).toBe(1536);
            expect(exportedData.globalSettings.codebaseIndexConfig.codebaseIndexOpenAiCompatibleBaseUrl).toBe("https://custom-api.example.com/v1");
        });
    });
});
//# sourceMappingURL=importExport.spec.js.map