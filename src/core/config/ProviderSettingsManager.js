...
import { providerSettingsWithIdSchema, discriminatedProviderSettingsWithIdSchema, isSecretStateKey, DEFAULT_CONSECUTIVE_MISTAKE_LIMIT, getModelId, } from "@roo-code/types";
...
export class ProviderSettingsManager {
    ...
    defaultModeApiConfigs = Object.fromEntries(modes.map((mode) => [mode.slug, this.defaultConfigId]));
    ...
    findUniqueProfileName(baseName, existingNames) {
    ...
    async syncCloudProfiles(cloudProfiles, currentActiveProfileName) {
        try {
            return await this.lock(async () => {
                const providerProfiles = await this.load();
                ...
                const newCloudIds = new Set(Object.values(cloudProfiles)
                    .map((p) => p.id)
                ...
                for (const [cloudName, cloudProfile] of Object.entries(cloudProfiles)) {
                    ...
                    const existingEntry = Object.entries(providerProfiles.apiConfigs).find(([_, profile]) => profile.id === cloudProfile.id);
                    if (existingEntry) {
...
//# sourceMappingURL=ProviderSettingsManager.js.map