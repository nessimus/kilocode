...
const isEmptyTextContent = (block) => block.type === "text" && block.content === "";
[parseAssistantMessageV1, parseAssistantMessageV2].forEach((parser, index) => {
    describe(`parseAssistantMessageV${index + 1}`, () => {
        ...
        describe("tool use parsing", () => {
            ...
            it("should parse a tool use with multiple parameters", () => {
                const message = "<read_file><path>src/file.ts</path><start_line>10</start_line><end_line>20</end_line></read_file>";
                const result = parser(message).filter((block) => !isEmptyTextContent(block));
                expect(result).toHaveLength(1);
                const toolUse = result[0];
                expect(toolUse.type).toBe("tool_use");
        ...
        describe("mixed content parsing", () => {
            ...
            it("should parse multiple tool uses separated by text", () => {
                const message = "First file: <read_file><path>src/file1.ts</path></read_file>Second file: <read_file><path>src/file2.ts</path></read_file>";
                const result = parser(message);
                ...
                expect(result[1].params.path).toBe("src/file1.ts");
                expect(result[2].type).toBe("text");
                expect(result[2].content).toBe("Second file:");
                expect(result[3].type).toBe("tool_use");
                expect(result[3].name).toBe("read_file");
                expect(result[3].params.path).toBe("src/file2.ts");
            });
        });
	...
	return true;
            ...
            it("should handle nested tool tags that aren't actually nested", () => {
                const message = "<execute_command><command>echo '<read_file><path>test.txt</path></read_file>'</command></execute_command>";
                const result = parser(message).filter((block) => !isEmptyTextContent(block));
                expect(result).toHaveLength(1);
                const toolUse = result[0];
                expect(toolUse.type).toBe("tool_use");
	...
	<execute_command><command>node src/index.ts</command></execute_command>`;
                ...
                // First text block
                expect(result[0].type).toBe("text");
                expect(result[0].content).toBe("I'll help you with that task.");
                // First tool use (read_file)
                expect(result[1].type).toBe("tool_use");
                expect(result[1].name).toBe("read_file");
                // Second text block
                expect(result[2].type).toBe("text");
                expect(result[2].content).toContain("Now let's modify the file:");
                // Second tool use (write_to_file)
                expect(result[3].type).toBe("tool_use");
                expect(result[3].name).toBe("write_to_file");
                // Third text block
                expect(result[4].type).toBe("text");
                expect(result[4].content).toContain("Let's run the code:");
                // Third tool use (execute_command)
                expect(result[5].type).toBe("tool_use");
                expect(result[5].name).toBe("execute_command");
            });
        });
    });
});
//# sourceMappingURL=parseAssistantMessage.spec.js.map