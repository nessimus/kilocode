...
const measureExecutionTime = (fn, input, iterations = 1000) => {
    for (let i = 0; i < 10; i++) {
        fn(input);
    }
...
const measureMemoryUsage = (fn, input, iterations = 100) => {
    if (global.gc) {
...
const runBenchmark = () => {
    ...
    console.log(`| ${"Test Case".padEnd(namePadding)} | V1 Time (ms) | V2 Time (ms) | V1/V2 Ratio | V1 Heap (bytes) | V2 Heap (bytes) |`);
    ...
    for (const testCase of testCases) {
        ...
        console.log(`| ${testCase.name.padEnd(namePadding)} | ` +
...
runBenchmark();
//# sourceMappingURL=parseAssistantMessageBenchmark.js.map