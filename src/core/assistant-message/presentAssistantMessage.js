 ...
 */
export async function presentAssistantMessage(cline, recursionDepth = 0 /*kilocode_change*/) {
    ...
    switch (block.type) {
        ...
        case "tool_use":
            const toolDescription = () => {
                switch (block.name) {
                    ...
                    case "create_employee":
                        return `[${block.name} for '${block.params.name}'${block.params.role ? ` as ${block.params.role}` : ""}]`;
                    case "update_company": {
                        const company = block.params.company_id ?? (block.params.company_updates ? "multiple" : "(unspecified)");
                        return `[${block.name} for company '${company}']`;
                    }
                    case "update_employee": {
                        const employee = block.params.employee_id ?? (block.params.employee_updates ? "multiple" : "(unspecified)");
                        const company = block.params.company_id ?? (block.params.employee_updates ? "multiple" : "(unspecified)");
                    ...
                    case "update_department": {
                        const department = block.params.department_id ??
                            (block.params.department_updates ? "multiple" : "(unspecified)");
                        const company = block.params.company_id ?? (block.params.department_updates ? "multiple" : "(unspecified)");
                    ...
                    case "update_team": {
                        ...
                        const company = block.params.company_id ?? (block.params.team_updates ? "multiple" : "(unspecified)");
                        return `[${block.name} for team '${team}' in company '${company}']`;
                    }
                    case "assign_employee_to_team": {
                        const employee = block.params.employee_id ?? (block.params.assignments ? "multiple" : "(unspecified)");
                    ...
                    case "assign_team_to_department": {
                        const team = block.params.team_id ?? (block.params.assignments ? "multiple" : "(unspecified)");
                        const department = block.params.department_id ?? (block.params.assignments ? "multiple" : "(detach)");
                    ...
                    case "archive_employee": {
                        const employee = block.params.employee_id ?? (block.params.employee_ids ? "multiple" : "(unspecified)");
                        const company = block.params.company_id ?? (block.params.employee_ids ? "multiple" : "(unspecified)");
                        return `[${block.name} for employee '${employee}' in company '${company}']`;
                    }
                    case "archive_team": {
                        const team = block.params.team_id ?? (block.params.team_ids ? "multiple" : "(unspecified)");
                        const company = block.params.company_id ?? (block.params.team_ids ? "multiple" : "(unspecified)");
                        return `[${block.name} for team '${team}' in company '${company}']`;
                    }
                    case "archive_department": {
                        const department = block.params.department_id ?? (block.params.department_ids ? "multiple" : "(unspecified)");
                        const company = block.params.company_id ?? (block.params.department_ids ? "multiple" : "(unspecified)");
                        return `[${block.name} for department '${department}' in company '${company}']`;
                    }
                    case "remove_employee_from_team": {
                        const employee = block.params.employee_id ?? (block.params.assignments ? "multiple" : "(unspecified)");
                        const team = block.params.team_id ?? (block.params.assignments ? "multiple" : "(unspecified)");
                        const company = block.params.company_id ?? (block.params.assignments ? "multiple" : "(unspecified)");
                        return `[${block.name} removing '${employee}' from '${team}' (${company})]`;
                    }
                    case "remove_team_from_department": {
                        const team = block.params.team_id ?? (block.params.assignments ? "multiple" : "(unspecified)");
                        const department = block.params.department_id ?? (block.params.assignments ? "multiple" : "(unspecified)");
                        const company = block.params.company_id ?? (block.params.assignments ? "multiple" : "(unspecified)");
            ...
            const pushToolResult = (content) => {
                ...
                cline
                    .say("tool_result", displayText, normalizedResult.images, false, undefined, undefined, { metadata: { toolResult: toolResultPayload } })
                    .catch((error) => {
            ...
            try {
                validateToolUse(block.name, mode ?? defaultModeSlug, customModes ?? [], { apply_diff: cline.diffEnabled }, block.params);
            }
            catch (error) {
            ...
            if (!block.partial) {
                ...
                if (!repetitionCheck.allowExecution && repetitionCheck.askUser) {
                    ...
                    const { response, text, images } = await cline.ask(repetitionCheck.askUser.messageKey, repetitionCheck.askUser.messageDetail.replace("{toolName}", block.name));
                    ...
                    pushToolResult(formatResponse.toolError(`Tool call repetition limit reached for ${block.name}. Please try a different approach.`));
                    break;
                }
            }
            switch (block.name) {
                ...
                case "read_file":
                    ...
                    const modelId = cline.api.getModel().id;
                    if (shouldUseSingleFileRead(modelId)) {
                        await simpleReadFileTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag);
                    }
                    else {
                ...
                case "attempt_completion":
                    ...
                    await attemptCompletionTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag, toolDescription, askFinishSubTaskApproval);
                    break;
                // kilocode_change start
                case "new_rule":
                ...
                case "update_team":
                    ...
                    break;
                case "assign_employee_to_team":
                    await assignEmployeeToTeamTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag);
                    break;
                case "assign_team_to_department":
                    await assignTeamToDepartmentTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag);
                    break;
                case "list_companies":
                ...
                case "archive_team":
                    ...
                    break;
                case "archive_department":
                    await archiveDepartmentTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag);
                    break;
                case "remove_employee_from_team":
                    await removeEmployeeFromTeamTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag);
                    break;
                case "remove_team_from_department":
                    await removeTeamFromDepartmentTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag);
                    break;
...
//# sourceMappingURL=presentAssistantMessage.js.map