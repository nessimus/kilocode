...
import type { WorkplaceEmployee } from "../../shared/golden/workplace"
...
function parseTraitsFlexible(value: unknown): string[] | undefined {
	if (value === undefined || value === null) {
		return undefined
	}
	if (typeof value === "string") {
...
function parseCustomAttributes(value: string | undefined): Record<string, string> | undefined {
	...
	return Object.fromEntries(
		Object.entries(parsed as Record<string, unknown>).map(([key, val]) => [key, val != null ? String(val) : ""]),
	)
}

function parseCustomAttributesFlexible(value: unknown): Record<string, string> | undefined {
	if (value === undefined || value === null) {
		return undefined
	}
	if (typeof value === "string") {
...
function parseMbtiFlexible(value: unknown): WorkplaceEmployee["mbtiType"] {
	const candidate = toOptionalStringFromUnknown(value)
	return parseMbti(candidate)
}

type EmployeeUpdateDescriptor = {
	companyId: string
	employeeId: string
	updates: {
		name?: string
		...
		description?: string
		personality?: string
		mbtiType?: WorkplaceEmployee["mbtiType"]
...
function parseEmployeeUpdatesJson(
	raw: string,
	defaults: {
		companyId?: string
		employeeId?: string
		...
		profileImageUrl?: string
		customAttributes?: Record<string, string>
		mbti?: WorkplaceEmployee["mbtiType"]
		isExecutiveManager?: boolean
	},
): EmployeeUpdateDescriptor[] {
	let parsed: unknown
	try {
	...
	return entries.map((entry, index) => {
		...
		const employeeId = toOptionalString(
			(record["employee_id"] as string | undefined) ??
		...
		let mbti: WorkplaceEmployee["mbtiType"] | undefined
		try {
			const entryMbti = parseMbtiFlexible(record["mbti_type"])
			mbti = entryMbti ?? defaults.mbti
		} catch (error) {
			throw new Error(
				`employee_updates[${index}] has invalid mbti_type: ${
		...
		try {
			...
			throw new Error(
				`employee_updates[${index}] has invalid personality_traits: ${
					error instanceof Error ? error.message : String(error)
				}`,
			)
		}
		...
		try {
			...
			throw new Error(
				`employee_updates[${index}] has invalid custom_attributes: ${
					error instanceof Error ? error.message : String(error)
				}`,
			)
		}

		let isExecutiveManager: boolean | undefined
		...
		if (rawIsExec !== undefined) {
			try {
				...
				throw new Error(
					`employee_updates[${index}] has invalid is_executive_manager: ${
						error instanceof Error ? error.message : String(error)
					}`,
				)
			}
		...
		const updates: EmployeeUpdateDescriptor["updates"] = {}
		if (name !== undefined) {
			updates.name = name
		}
		if (role !== undefined) {
		...
		if (Object.keys(updates).length === 0) {
			throw new Error(
				`employee_updates[${index}] requires at least one field to update (name, role, description, personality, mbti_type, personality_traits, profile_image_url, custom_attributes, or is_executive_manager)`,
			)
		}
...
export async function updateEmployeeTool(
	cline: Task,
	block: ToolUse,
	...
	try {
		...
		let defaultTraits: string[] | undefined
		...
		let descriptors: EmployeeUpdateDescriptor[]
		try {
			if (employeeUpdatesRaw) {
				...
				const updates: EmployeeUpdateDescriptor["updates"] = {}
				if (defaultName !== undefined) updates.name = defaultName
				if (defaultRole !== undefined) updates.role = defaultRole
				if (defaultDescription !== undefined) updates.description = defaultDescription
				if (defaultPersonality !== undefined) updates.personality = defaultPersonality
				if (defaultMbti !== undefined) updates.mbtiType = defaultMbti
				if (defaultTraits !== undefined) updates.personalityTraits = defaultTraits
				if (defaultProfileImageUrl !== undefined) updates.profileImageUrl = defaultProfileImageUrl
				if (defaultCustomAttributes !== undefined) updates.customAttributes = defaultCustomAttributes
				if (defaultIsExecutiveManager !== undefined) updates.isExecutiveManager = defaultIsExecutiveManager

				if (Object.keys(updates).length === 0) {
					throw new Error(
						"Provide at least one field to update (name, role, description, personality, mbti_type, personality_traits, profile_image_url, custom_attributes, or is_executive_manager).",
					)
				}
		...
		const updatedSummaries: Array<{ companyId: string; employeeId: string; name: string }> = []
		for (const descriptor of descriptors) {
			...
			const updatedEmployee: WorkplaceEmployee = {
				...employee,
		...
		await handleError("update employee", error as Error)
	}
}
