...
function calculateMorphCost(inputTokens, outputTokens, model) {
    const normalizedModel = model.replace("morph/", ""); // Remove OpenRouter prefix if present
    const pricing = MORPH_MODEL_PRICING[normalizedModel] || MORPH_MODEL_PRICING["auto"];
...
export async function editFileTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    ...
    try {
        ...
        const approved = await askApproval("tool", JSON.stringify({
            ...
            fastApplyResult: morphApplyResult
                ...
                : undefined,
        }), undefined, cline.rooProtectedController?.isWriteProtected(relPath) || false);
        if (!approved) {
        ...
        await handleError("editing file with Morph", error);
        await cline.diffViewProvider.reset();
    }
}
async function applyMorphEdit(originalContent, instructions, codeEdit, cline, filePath) {
    try {
...
function getMorphConfiguration(state) {
...
export function isMorphAvailable(state) {
