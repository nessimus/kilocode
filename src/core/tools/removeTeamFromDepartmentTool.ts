...
export async function removeTeamFromDepartmentTool(
	cline: Task,
	block: ToolUse,
	...
	try {
		...
		const summary = (() => {
			if (summaries.length === 1) {
				...
				return `Detached ${single.teamName ?? single.teamId} from ${
					single.departmentName ?? single.departmentId
				} in ${single.companyId}.`
			}
			const details = summaries
				.map(
					(entry) =>
						`${entry.teamName ?? entry.teamId} x ${entry.departmentName ?? entry.departmentId} (${entry.companyId})`,
				)
				.join(", ")
			return `Detached ${summaries.length} team/department pairs across ${
		...
		await handleError("remove team from department", error as Error)
	}
}
