...
export async function removeEmployeeFromTeamTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    ...
    try {
        ...
        const summary = (() => {
            if (summaries.length === 1) {
                ...
                return `Removed ${single.employeeName ?? single.employeeId} from ${single.teamName ?? single.teamId} in ${single.companyId}.`;
            }
            const details = summaries
                .map((entry) => `${entry.employeeName ?? entry.employeeId} ← ${entry.teamName ?? entry.teamId} (${entry.companyId})`)
                .join(", ");
            return `Removed ${summaries.length} team memberships across ${new Set(summaries.map((entry) => entry.companyId)).size} companies: ${details}.`;
        })();
        pushToolResult(formatResponse.toolResult(summary));
    }
    catch (error) {
        await handleError("remove employee from team", error);
    }
}
//# sourceMappingURL=removeEmployeeFromTeamTool.js.map