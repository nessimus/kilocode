...
import {
	...
	normalizeOwnerProfileInput,
} from "./helpers/workplace"

interface CompanyEntryInput {
	name?: unknown
	emoji?: unknown
	description?: unknown
	vision?: unknown
	...
	update_default_owner_profile?: unknown
}
...
function parseCompaniesJson(
	...
	return entries.map((entry, index) => {
		...
		return {
			name: (entry as Record<string, unknown>)["name"] ?? defaults.name,
			emoji: (entry as Record<string, unknown>)["emoji"] ?? defaults.emoji,
			description:
				(entry as Record<string, unknown>)["description"] ?? defaults.description,
			vision: (entry as Record<string, unknown>)["vision"] ?? defaults.vision,
			mission: (entry as Record<string, unknown>)["mission"] ?? defaults.mission,
			owner_profile:
				(entry as Record<string, unknown>)["owner_profile"] ??
				(entry as Record<string, unknown>)["ownerProfile"] ??
				defaults.ownerProfile,
			update_default_owner_profile:
				(entry as Record<string, unknown>)["update_default_owner_profile"] ??
				(entry as Record<string, unknown>)["updateDefaultOwnerProfile"] ??
...
function buildCreatePayload(
	...
	let ownerProfile: WorkplaceOwnerProfile | undefined
	...
	let updateDefaultOwnerProfile: boolean | undefined
	try {
		updateDefaultOwnerProfile = normalizeBooleanInput(
			entry.update_default_owner_profile,
			"update_default_owner_profile",
		)
	} catch (error) {
		const message = error instanceof Error ? error.message : String(error)
	...
	try {
		if (block.partial) {
			const partialMessage = JSON.stringify({
				...
				update_default_owner_profile: removeClosingTag(
					"update_default_owner_profile",
					updateDefaultsRaw,
				),
		...
		let defaultOwnerProfile: WorkplaceOwnerProfile | undefined
		try {
		...
		let defaultUpdateOwnerProfile: boolean | undefined
		try {
			defaultUpdateOwnerProfile = parseBooleanFlag(
				updateDefaultsRaw,
				"update_default_owner_profile",
			)
		} catch (error) {
		...
		let companyInputs: CompanyEntryInput[]
		...
		const payloads = companyInputs.map((entry, index) => (
			buildCreatePayload(entry, companyInputs.length > 1 ? index : -1)
		))
		...
		const createdSummaries: Array<{ id?: string; name: string; vision?: string; mission?: string }> = []
		const existingIds = new Set(
			workplaceService.getState().companies.map((company) => company.id),
		)
		...
		if (error instanceof Error) {
			...
			await handleError("create company", error as Error)
		}
	}
}
