...
export async function listDepartmentsTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    ...
    try {
        ...
        const lines = limited.map((entry) => {
            ...
            return `• ${entry.name} — id: ${entry.departmentId}, company: ${entry.companyId}, teams: ${entry.teamCount}${suffix}${entry.isArchived ? " (archived)" : ""}`;
        });
        ...
        await handleError("list departments", error);
    }
}
//# sourceMappingURL=listDepartmentsTool.js.map