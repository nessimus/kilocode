...
interface CompanyUpdateEntryInput {
	company_id?: unknown
	name?: unknown
	emoji?: unknown
	description?: unknown
...
function parseCompanyUpdatesJson(
	raw: string,
	defaults: {
		companyId?: string
		name?: string
	...
	return entries.map((entry, index) => {
		...
		return {
			...
			update_default_owner_profile:
				record["update_default_owner_profile"] ??
...
export async function updateCompanyTool(
	...
	try {
		...
		let updateInputs: CompanyUpdateEntryInput[]
		...
		const payloads = updateInputs.map((entry, index) => (
			buildUpdatePayload(entry, updateInputs.length > 1 ? index : -1)
		))
		...
		const updatedSummaries: Array<{ id: string; name?: string }> = []
		...
		if (error instanceof Error) {
			...
			await handleError("update company", error as Error)
		}
	}
}
