...
export async function generateImageTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    ...
    if (!isImageGenerationEnabled) {
        pushToolResult(formatResponse.toolError("Image generation is an experimental feature that must be enabled in settings. Please enable 'Image Generation' in the Experimental Settings section."));
        return;
    }
    if (block.partial) {
        return;
    ...
    if (inputImagePath) {
        ...
        if (!inputImageExists) {
            ...
            pushToolResult(formatResponse.toolError(`Input image not found: ${getReadablePath(cline.cwd, inputImagePath)}`));
            return;
        }
        ...
        try {
            ...
            if (!supportedFormats.includes(imageExtension)) {
                await cline.say("error", `Unsupported image format: ${imageExtension}. Supported formats: ${supportedFormats.join(", ")}`);
                pushToolResult(formatResponse.toolError(`Unsupported image format: ${imageExtension}. Supported formats: ${supportedFormats.join(", ")}`));
                return;
            }
            ...
            await cline.say("error", `Failed to read input image: ${error instanceof Error ? error.message : "Unknown error"}`);
            pushToolResult(formatResponse.toolError(`Failed to read input image: ${error instanceof Error ? error.message : "Unknown error"}`));
            return;
        }
    }
    ...
    if (!openRouterApiKey && !kiloCodeApiKey) {
        await cline.say("error", "OpenRouter API key is required for image generation. Please configure it in the Image Generation experimental settings.");
        pushToolResult(formatResponse.toolError("OpenRouter API key is required for image generation. Please configure it in the Image Generation experimental settings."));
        return;
    }
        ...
        if (!block.partial) {
            ...
            const openRouterHandler = openRouterApiKey
                ...
                : new KilocodeOpenrouterHandler({
                    ...
                    kilocodeOrganizationId: cline.apiConfiguration.apiProvider === "kilocode" &&
            ...
            const result = await openRouterHandler.generateImage(prompt, selectedModel, 
            // kilocode_change start
            openRouterApiKey ||
                kiloCodeApiKey ||
                (() => {
