...
export async function upsertSopTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    const rawName = block.params.sop_name ?? block.params.name;
    const rawContent = block.params.content;
    const looseParams = block.params;
    ...
    try {
        ...
        await handleError("upserting SOP", error);
    }
}
//# sourceMappingURL=upsertSopTool.js.map