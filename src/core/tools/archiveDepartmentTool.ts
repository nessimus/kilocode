...
function parseArchiveJson(raw: string, fallbackCompanyId?: string, fallbackDepartmentId?: string): ArchiveDescriptor[] {
	let parsed: unknown
	try {
...
export async function archiveDepartmentTool(
	cline: Task,
	block: ToolUse,
	...
	try {
		...
		let archives: ArchiveDescriptor[]
		try {
			if (departmentIdsRaw) {
		...
		const summaries: Array<{ companyId: string; departmentId: string; name?: string }> = []
		...
		const summary = (() => {
			...
			const details = summaries
				...
				.join(", ")
			return `Archived ${summaries.length} departments across ${
		...
		await handleError("archive department", error as Error)
	}
}
