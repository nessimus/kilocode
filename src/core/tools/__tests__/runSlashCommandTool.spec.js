...
describe("runSlashCommandTool", () => {
    ...
    it("should handle missing command parameter", async () => {
        ...
        await runSlashCommandTool(mockTask, block, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
    ...
    it("should handle command not found", async () => {
        ...
        await runSlashCommandTool(mockTask, block, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockTask.recordToolError).toHaveBeenCalledWith("run_slash_command");
        expect(mockPushToolResult).toHaveBeenCalledWith(formatResponse.toolError("Standard Operating Procedure 'nonexistent' not found. Document SOPs: init, test, deploy. Workflow SOPs: (none)."));
    });
    ...
    it("should successfully execute built-in command", async () => {
        ...
        expect(mockPushToolResult).toHaveBeenCalledWith(`Standard Operating Procedure: init
...
Use this document as a reference and narrate how each step applies to the current task.`);
    ...
    it("should successfully execute command with arguments", async () => {
        ...
        expect(mockPushToolResult).toHaveBeenCalledWith(`Standard Operating Procedure: test
Type: Loose document guidance
Source: project
...
Use this document as a reference and narrate how each step applies to the current task.`);
    ...
    it("should handle global command", async () => {
        ...
        vi.mocked(getCommand).mockResolvedValue(mockCommand);
        await runSlashCommandTool(mockTask, block, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockPushToolResult).toHaveBeenCalledWith(`Standard Operating Procedure: deploy
Type: Loose document guidance
Source: global
...
Use this document as a reference and narrate how each step applies to the current task.`);
    ...
    it("should resolve workflow SOP when document is missing", async () => {
        ...
        await runSlashCommandTool(mockTask, block, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockPushToolResult).toHaveBeenCalledWith(`Standard Operating Procedure: publish.workflow.json
...
Follow this workflow sequentially, honoring each node's inputs and outputs before advancing.`);
    ...
    it("should handle empty available commands list", async () => {
        ...
        vi.mocked(getCommandNames).mockResolvedValue([]);
        await runSlashCommandTool(mockTask, block, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockPushToolResult).toHaveBeenCalledWith(formatResponse.toolError("Command 'nonexistent' not found. Available commands: (none)"));
    });
