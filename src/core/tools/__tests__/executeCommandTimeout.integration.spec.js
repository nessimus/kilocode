...
import { executeCommand, executeCommandTool } from "../executeCommandTool";
...
describe("Command Execution Timeout Integration", () => {
    ...
    beforeEach(() => {
        ...
        TerminalRegistry.getOrCreateTerminal.mockResolvedValue(mockTerminal);
    ...
    it("should handle timeout scenario", async () => {
        ...
        const result = await executeCommand(mockTask, options);
        ...
        it("should skip timeout for commands in allowlist", async () => {
            ...
            const mockGetConfiguration = vitest.fn().mockReturnValue({
                get: vitest.fn().mockImplementation((key) => {
                    ...
                    if (key === "commandTimeoutAllowlist")
                        return ["npm", "git"];
                    return undefined;
                }),
            });
            ...
            mockTerminal.runCommand.mockReturnValue(longRunningProcess);
            await executeCommandTool(mockTask, mockBlock, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
