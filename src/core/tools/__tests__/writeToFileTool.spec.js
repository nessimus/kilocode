...
vi.mock("../../prompts/responses", () => ({
    formatResponse: {
        ...
        lineCountTruncationError: vi.fn((count, isNew, diffEnabled) => `Line count error: ${count}, new: ${isNew}, diff: ${diffEnabled}`),
...
vi.mock("../../../integrations/misc/extract-text", () => ({
    ...
    addLineNumbers: vi.fn().mockImplementation((content) => content
        .split("\n")
        .map((line, i) => `${i + 1} | ${line}`)
        .join("\n")),
}));
...
describe("writeToFileTool", () => {
    ...
    const mockedFileExistsAtPath = fileExistsAtPath;
    const mockedDetectCodeOmission = detectCodeOmission;
    const mockedIsPathOutsideWorkspace = isPathOutsideWorkspace;
    const mockedGetReadablePath = getReadablePath;
    const mockedUnescapeHtmlEntities = unescapeHtmlEntities;
    const mockedEveryLineHasLineNumbers = everyLineHasLineNumbers;
    const mockedStripLineNumbers = stripLineNumbers;
    ...
    beforeEach(() => {
        ...
        mockCline.diffViewProvider = {
            ...
            pushToolWriteResult: vi.fn().mockImplementation(async function (task, cwd, isNewFile) {
                ...
                if (this.userEdits) {
                    await task.say("user_feedback_diff", JSON.stringify({
    ...
    async function executeWriteFileTool(params = {}, options = {}) {
        ...
        await writeToFileTool(mockCline, toolUse, mockAskApproval, mockHandleError, (result) => {
            toolResult = result;
    ...
    describe("user interaction", () => {
        ...
        it("reports user edits with diff feedback", async () => {
            ...
            await executeWriteFileTool({}, { fileExists: true });
            expect(mockCline.say).toHaveBeenCalledWith("user_feedback_diff", expect.stringContaining("editedExistingFile"));
        });
    });
