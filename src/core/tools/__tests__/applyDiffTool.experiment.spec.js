...
describe("applyDiffTool experiment routing", () => {
    ...
    it("should use legacy tool when MULTI_FILE_APPLY_DIFF experiment is disabled", async () => {
        ...
        applyDiffToolLegacy.mockResolvedValue(undefined);
        await applyDiffTool(mockCline, mockBlock, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(applyDiffToolLegacy).toHaveBeenCalledWith(mockCline, mockBlock, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
    });
