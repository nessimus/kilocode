...
const mockRemoveClosingTag = vi.fn((_name, value) => value ?? "");
...
describe("newTaskTool", () => {
    ...
    it("should correctly un-escape \\\\@ to \\@ in the message passed to the new task", async () => {
        ...
        await newTaskTool(mockCline, // Use 'as any' for simplicity in mocking complex type
        block, mockAskApproval, // Now correctly typed
        mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        // Verify askApproval was called
        expect(mockAskApproval).toHaveBeenCalled();
        // Verify the message passed to startSubtask reflects the code's behavior in unit tests
        expect(mockStartSubtask).toHaveBeenCalledWith("Review this: \\@file1.txt and also \\\\\\@file2.txt", // Unit Test Expectation: \\@ -> \@, \\\\@ -> \\\\@
    ...
    it("should not un-escape single escaped \@", async () => {
        ...
        await newTaskTool(mockCline, block, mockAskApproval, // Now correctly typed
        mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockStartSubtask).toHaveBeenCalledWith("This is already unescaped: \\@file1.txt", // Expected: \@ remains \@
        expect.any(Array), "code");
    });
    it("should not un-escape non-escaped @", async () => {
        ...
        await newTaskTool(mockCline, block, mockAskApproval, // Now correctly typed
        mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockStartSubtask).toHaveBeenCalledWith("A normal mention @file1.txt", // Expected: @ remains @
        expect.any(Array), "code");
    });
    it("should handle mixed escaping scenarios", async () => {
        ...
        await newTaskTool(mockCline, block, mockAskApproval, // Now correctly typed
        mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockStartSubtask).toHaveBeenCalledWith("Mix: @file0.txt, \\@file1.txt, \\@file2.txt, \\\\\\@file3.txt", // Unit Test Expectation: @->@, \@->\@, \\@->\@, \\\\@->\\\\@
        expect.any(Array), "code");
    });
    it("should handle missing todos parameter gracefully (backward compatibility)", async () => {
        ...
        await newTaskTool(mockCline, block, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
    ...
    it("should work with todos parameter when provided", async () => {
        ...
        expect(mockStartSubtask).toHaveBeenCalledWith("Test message with todos", expect.arrayContaining([
            expect.objectContaining({ content: "First task" }),
            expect.objectContaining({ content: "Second task" }),
        ]), "code");
        expect(mockPushToolResult).toHaveBeenCalledWith(expect.stringContaining("Successfully created new task"));
    ...
    it("should parse todos with different statuses correctly", async () => {
        ...
        await newTaskTool(mockCline, block, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockStartSubtask).toHaveBeenCalledWith("Test message", expect.arrayContaining([
    ...
    describe("VSCode setting: newTaskRequireTodos", () => {
        ...
        it("should REQUIRE todos when VSCode setting is enabled", async () => {
            ...
            expect(mockPushToolResult).not.toHaveBeenCalledWith(expect.stringContaining("Successfully created new task"));
        });
        ...
        it("should use current Package.name value (roo-code-nightly) when accessing VSCode configuration", async () => {
            ...
            pkg.Package.name = "roo-code-nightly";
            const block = {
                type: "tool_use",
                name: "new_task",
                params: {
...
//# sourceMappingURL=newTaskTool.spec.js.map