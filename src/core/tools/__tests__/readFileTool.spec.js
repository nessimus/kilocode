...
const { toolResultMock, imageBlocksMock } = vi.hoisted(() => {
    const toolResultMock = vi.fn((text, images) => {
    ...
    const imageBlocksMock = vi.fn((images) => {
        return images
            ? images.map((img) => {
                const [header, data] = img.split(",");
                const media_type = header.match(/:(.*?);/)?.[1] || "image/png";
...
vi.mock("../../prompts/responses", () => ({
    formatResponse: {
        ...
        toolDeniedWithFeedback: vi.fn((feedback) => `The user denied this operation and provided the following feedback:\n<feedback>\n${feedback}\n</feedback>`),
        toolApprovedWithFeedback: vi.fn((feedback) => `The user approved this operation and provided the following context:\n<feedback>\n${feedback}\n</feedback>`),
        rooIgnoreError: vi.fn((path) => `Access to ${path} is blocked by the .kilocodeignore file settings. You must try to continue in the task without using this file, or ask the user to update the .kilocodeignore file.`),
...
beforeEach(() => {
    ...
    toolResultMock.mockImplementation((text, images) => {
        if (images && images.length > 0) {
            return [
                { type: "text", text },
                ...images.map((img) => {
    ...
    imageBlocksMock.mockImplementation((images) => {
        return images
            ? images.map((img) => {
                const [header, data] = img.split(",");
                const media_type = header.match(/:(.*?);/)?.[1] || "image/png";
...
vi.mock("../../../i18n", () => ({
    t: vi.fn((key, params) => {
        ...
        const translations = {
            ...
            "tools:readFile.imageTooLarge": "Image file is too large ({{size}}). The maximum allowed size is {{max}} MB.",
...
function setImageSupport(mockCline, supportsImages) {
    mockCline.api = {
        getModel: vi.fn().mockReturnValue({
            info: {
                supportsImages,
...
describe("read_file tool with maxReadFileLine setting", () => {
     ...
     */
    async function executeReadFileTool(params = {}, options = {}) {
        ...
        await readFileTool(mockCline, toolUse, mockCline.ask, vi.fn(), (result) => {
            toolResult = result;
        }, (_, content) => content ?? "");
        return toolResult;
    }
...
describe("read_file tool XML output structure", () => {
    ...
    describe("Basic XML Structure Tests", () => {
        ...
        it("should follow the correct XML structure format", async () => {
            ...
            const xmlStructureRegex = new RegExp(`^<files>\\n<file><path>${testFilePath}</path>\\n<content lines="1-5">\\n.*</content>\\n</file>\\n</files>$`, "s");
        ...
        describe("Total Image Memory Limit", () => {
            ...
            async function executeReadMultipleImagesTool(imagePaths) {
            ...
            it("should handle individual image size limit and total memory limit together", async () => {
                ...
                expect(textPart).toMatch(/Image file is too large \(\d+(\.\d+)? MB\)\. The maximum allowed size is 5 MB\./);
            });
...
describe("read_file tool with image support", () => {
    ...
    const base64ImageData = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==";
    ...
    async function executeReadImageTool(imagePath = testImagePath) {
        ...
        await readFileTool(localMockCline, toolUse, localMockCline.ask, vi.fn(), (result) => {
            toolResult = result;
        }, (_, content) => content ?? "");
    ...
    describe("Image Reading Functionality", () => {
        ...
        it("should handle errors when reading image files", async () => {
            ...
            await readFileTool(localMockCline, toolUse, localMockCline.ask, handleErrorSpy, // Use our spy here
            (result) => {
                toolResult = result;
            }, (_, content) => content ?? "");
...
//# sourceMappingURL=readFileTool.spec.js.map