...
import type { WorkplaceDepartment } from "../../shared/golden/workplace"
...
type DepartmentUpdateDescriptor = {
	companyId: string
	departmentId: string
	name?: string
	description?: string
...
function parseDepartmentUpdatesJson(
	raw: string,
	fallbackCompanyId?: string,
	fallbackDepartmentId?: string,
	fallbackName?: string,
	fallbackDescription?: string,
): DepartmentUpdateDescriptor[] {
	let parsed: unknown
	try {
	...
	return entries.map((entry, index) => {
		...
		const departmentId = toOptionalString(
			(record["department_id"] as string | undefined) ??
				(record["departmentId"] as string | undefined) ??
...
export async function updateDepartmentTool(
	cline: Task,
	block: ToolUse,
	...
	try {
		...
		let updateDescriptors: DepartmentUpdateDescriptor[]
		try {
			if (updatesRaw) {
				updateDescriptors = parseDepartmentUpdatesJson(
					updatesRaw,
					defaultCompanyId,
					defaultDepartmentId,
					defaultName,
		...
		const updatedSummaries: Array<{ companyId: string; departmentId: string; name: string }> = []
		for (const descriptor of updateDescriptors) {
			...
			const updatedDepartment: WorkplaceDepartment = {
				...department,
		...
		await handleError("update department", error as Error)
	}
}
