...
import { type CreateDepartmentPayload } from "../../shared/golden/workplace"
...
export async function createDepartmentTool(
	cline: Task,
	block: ToolUse,
	...
	try {
		...
		const buildSinglePayloads = (): CreateDepartmentPayload[] => {
			if (!defaultCompanyId) {
		...
		let departmentPayloads: CreateDepartmentPayload[]
		try {
			if (departmentsRaw) {
				departmentPayloads = parseDepartmentsJson(
					departmentsRaw,
					defaultCompanyId,
					defaultName,
					defaultDescription,
		...
		const createdSummaries: { companyId: string; name: string }[] = []
		...
		await handleError("create department", error as Error)
	}
}
