...
export async function assignEmployeeToTeamTool(
	cline: Task,
	block: ToolUse,
	...
	try {
		...
		let assignments: AssignmentDescriptor[]
		try {
			if (assignmentsRaw) {
		...
		const summaries: Array<{ companyId: string; teamId: string; employeeId: string }> = []
		...
		const summary = (() => {
			...
			const details = summaries
				...
				.join(", ")
			return `Assigned ${summaries.length} employees across ${
		...
		await handleError("assign employee to team", error as Error)
	}
}
