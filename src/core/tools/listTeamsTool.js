...
export async function listTeamsTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    ...
    try {
        ...
        const lines = limited.map((entry) => {
            ...
            return `• ${entry.name} — id: ${entry.teamId}, company: ${entry.companyId}${departmentText}, members: ${entry.memberCount}${entry.isArchived ? " (archived)" : ""}`;
        });
        ...
        await handleError("list teams", error);
    }
}
