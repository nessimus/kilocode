...
function parseActionItemsJson(raw, defaults) {
    let parsed;
    try {
    ...
    return entries.map((entry, index) => {
        ...
        const kind = parseActionItemKind(record["kind"] ?? record["type"], `action_items[${index}].kind`) ?? defaults.kind;
        if (!kind) {
        ...
        const description = toOptionalStringFromUnknown(record["description"] ?? record["details"]) ?? defaults.description;
        const ownerEmployeeId = toOptionalStringFromUnknown(record["owner_employee_id"] ?? record["ownerEmployeeId"]) ??
        ...
        try {
            const parsedPriority = parsePriority(record["priority"] ?? record["importance"] ?? defaults.priority, `action_items[${index}].priority`);
        ...
        try {
            const parsedProperties = parseCustomProperties(record["custom_properties"] ?? record["customProperties"] ?? defaults.customProperties);
...
export async function createActionItemTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    ...
    try {
        ...
        for (const descriptor of descriptors) {
            ...
            const companyKnownIds = knownIds.get(descriptor.companyId) ?? new Set();
            let createdItemId;
            let statusName;
            if (company) {
        ...
        if (summaries.length === 1) {
            ...
            pushToolResult(formatResponse.toolResult(`Created action item ${single.title}${idText} in company ${single.companyId}.${statusText}${ownerText}${priorityText}`.trim()));
            return;
        }
        ...
        const details = summaries
            ...
            .join("; ");
        pushToolResult(formatResponse.toolResult(`Created ${summaries.length} action items across ${companySet.size} companies: ${details}.`));
    }
    catch (error) {
        await handleError("create action item", error);
    }
}
//# sourceMappingURL=createActionItemTool.js.map