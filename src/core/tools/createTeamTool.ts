...
import { type CreateTeamPayload } from "../../shared/golden/workplace"
...
function parseTeamsJson(
	raw: string,
	fallbackCompanyId?: string,
	fallbackDepartmentId?: string,
	fallbackName?: string,
	fallbackDescription?: string,
): Array<CreateTeamPayload> {
	let parsed: unknown
	try {
	...
	return entries.map((entry, index) => {
		...
		const departmentId = toOptionalString(
			(record["department_id"] ?? record["departmentId"] ?? fallbackDepartmentId) as string | undefined,
		)
...
export async function createTeamTool(
	cline: Task,
	block: ToolUse,
	...
	try {
		...
		const buildSinglePayloads = (): CreateTeamPayload[] => {
			if (!defaultCompanyId) {
		...
		let teamPayloads: CreateTeamPayload[]
		try {
			if (teamsRaw) {
				teamPayloads = parseTeamsJson(
					teamsRaw,
					defaultCompanyId,
					defaultDepartmentId,
					defaultName,
		...
		const createdSummaries: Array<{ companyId: string; name: string; departmentId?: string }> = []
		...
		const summary = (() => {
			...
			const details = createdSummaries
				.map(
					(entry) =>
						`${entry.name} (${entry.companyId}${entry.departmentId ? ` -> ${entry.departmentId}` : ""})`,
				)
				.join(", ")
		...
		await handleError("create team", error as Error)
	}
}
