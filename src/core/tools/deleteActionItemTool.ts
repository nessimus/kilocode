...
function parseDeleteJson(raw: string, fallbackCompanyId?: string, fallbackActionItemId?: string): DeleteDescriptor[] {
	let parsed: unknown
	try {
	...
	if (Array.isArray(parsed) && parsed.every((entry) => typeof entry === "string")) {
		...
		return (parsed as string[]).map((id, index) => {
	...
	return entries.map((entry, index) => {
		...
		const actionItemId =
			toOptionalStringFromUnknown(record["action_item_id"] ?? record["actionItemId"]) ?? fallbackActionItemId
		if (!actionItemId) {
...
export async function deleteActionItemTool(
	cline: Task,
	block: ToolUse,
	...
	try {
		...
		let descriptors: DeleteDescriptor[]
		try {
		...
		const summaries: Array<{ companyId: string; actionItemId: string; title?: string }> = []
		...
		if (summaries.length === 1) {
			...
			pushToolResult(
				formatResponse.toolResult(
					`Deleted action item${titleText} (${single.actionItemId}) from company ${single.companyId}.`,
				),
			)
			return
		}
		...
		pushToolResult(
			formatResponse.toolResult(
				`Deleted ${summaries.length} action items across ${companySet.size} companies: ${details}.`,
			),
		)
	} catch (error) {
		await handleError("delete action item", error as Error)
	}
}
