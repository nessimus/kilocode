...
export async function applyDiffToolLegacy(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    const relPath = block.params.path;
    let diffContent = block.params.diff;
    ...
    try {
        if (block.partial) {
            ...
            const diffResult = (await cline.diffStrategy?.applyDiff(originalContent, diffContent, parseInt(block.params.start_line ?? ""))) ?? {
                success: false,
            ...
            if (isPreventFocusDisruptionEnabled) {
                ...
                await cline.diffViewProvider.saveDirectly(relPath, diffResult.content, false, diagnosticsEnabled, writeDelayMs);
            }
            else {
