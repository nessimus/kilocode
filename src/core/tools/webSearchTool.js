...
import { SearchRouter, WebSearchError } from "../../services/search";
const DEFAULT_SEARCH_TYPE = "quick";
const MAX_RESULTS_TO_DISPLAY = 8;
export async function webSearchTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    ...
    const searchType = normalizeSearchType(searchTypeRaw ? removeClosingTag("web_search_type", searchTypeRaw) : undefined);
    ...
    try {
        ...
        const request = { query, searchType, context };
        ...
        await handleError("web search", error);
    }
}
async function buildSearchRouter(cline) {
    let tavilyKey;
    let braveKey;
    let perplexityKey;
    ...
    if (provider && typeof provider.getState === "function") {
        try {
            ...
            const searchSettings = state?.webSearchSettings ?? state?.webSearch ?? undefined;
            if (searchSettings) {
...
function formatWebSearchResult(query, searchType, result) {
    const lines = result.results.slice(0, MAX_RESULTS_TO_DISPLAY).map((item, index) => {
        ...
        const providerDetails = [item.provider];
