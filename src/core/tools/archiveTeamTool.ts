...
function parseArchiveJson(raw: string, fallbackCompanyId?: string, fallbackTeamId?: string): ArchiveDescriptor[] {
	let parsed: unknown
	try {
...
export async function archiveTeamTool(
	cline: Task,
	block: ToolUse,
	...
	try {
		...
		const summaries: Array<{ companyId: string; teamId: string; name?: string }> = []
		...
		const summary = (() => {
			...
			const details = summaries
				...
				.join(", ")
			return `Archived ${summaries.length} teams across ${
		...
		await handleError("archive team", error as Error)
	}
}
