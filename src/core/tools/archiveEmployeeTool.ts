...
function parseArchiveJson(raw: string, fallbackCompanyId?: string, fallbackEmployeeId?: string): ArchiveDescriptor[] {
	let parsed: unknown
	try {
...
export async function archiveEmployeeTool(
	cline: Task,
	block: ToolUse,
	...
	try {
		...
		const summaries: Array<{ companyId: string; employeeId: string; name?: string }> = []
		...
		const summary = (() => {
			...
			const details = summaries
				...
				.join(", ")
			return `Archived ${summaries.length} employees across ${
		...
		await handleError("archive employee", error as Error)
	}
}
