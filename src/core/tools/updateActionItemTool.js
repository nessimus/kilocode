...
const ACTION_ITEM_KINDS = ["goal", "project", "task"];
...
function parseActionItemKind(value, context) {
    const candidate = toOptionalStringFromUnknown(value);
    if (!candidate) {
        return undefined;
    }
    const normalized = candidate.toLowerCase();
    if (ACTION_ITEM_KINDS.includes(normalized)) {
        return normalized;
    }
    throw new Error(`${context} must be one of: ${ACTION_ITEM_KINDS.join(", ")}`);
}
function parsePriority(value, context) {
    const candidate = toOptionalStringFromUnknown(value);
    if (!candidate) {
        return undefined;
    }
...
function parsePriorityUpdate(value, context) {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
...
function parseDueAt(value, context) {
    const candidate = toOptionalStringFromUnknown(value);
    if (!candidate) {
        return undefined;
    }
...
function parseDueAtUpdate(value, context) {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
...
function normalizeCustomProperties(record) {
    const result = {};
...
function parseCustomProperties(value) {
    ...
    if (typeof value === "object" && !Array.isArray(value)) {
        return normalizeCustomProperties(value);
    }
    if (typeof value === "string") {
        const trimmed = value.trim();
        if (!trimmed) {
        ...
        return normalizeCustomProperties(parsed);
    }
    throw new Error("custom_properties must be a JSON object");
}
function parseCustomPropertiesUpdate(value) {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
...
function ensureHasMutations(updates, context) {
...
function parseUpdatesJson(raw, defaults) {
    let parsed;
    try {
    ...
    return entries.map((entry, index) => {
        ...
        const actionItemId = toOptionalStringFromUnknown(record["action_item_id"] ?? record["actionItemId"]) ?? defaults.actionItemId;
        if (!actionItemId) {
        ...
        try {
            kind =
                parseActionItemKind(record["kind"] ?? record["type"], `action_item_updates[${index}].kind`) ??
                    defaults.kind;
            priority = parsePriorityUpdate(record["priority"] ?? record["importance"], `action_item_updates[${index}].priority`);
...
export async function updateActionItemTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    ...
    try {
        ...
        let defaultPriority;
        let defaultDueAt;
        let defaultCustomProperties;
        let defaultDescription;
        let defaultOwnerEmployeeId;
        try {
        ...
        for (const descriptor of descriptors) {
            ...
            const next = {
                ...
                ownerEmployeeId: updates.ownerEmployeeId === undefined
                ...
                customProperties: updates.customProperties === undefined
        ...
        if (summaries.length === 1) {
            ...
            const ownerText = single.ownerEmployeeId !== undefined ? ` owner=${single.ownerEmployeeId || "(unassigned)"}` : "";
            const priorityText = single.priority ? ` priority=${single.priority}` : "";
            pushToolResult(formatResponse.toolResult(`Updated action item ${single.title} (${single.actionItemId}) in company ${single.companyId}.${statusText}${ownerText}${priorityText}${changes}`));
            return;
        }
        ...
        const details = summaries
            ...
            .join("; ");
        pushToolResult(formatResponse.toolResult(`Updated ${summaries.length} action items across ${companySet.size} companies: ${details}.`));
    }
    catch (error) {
        await handleError("update action item", error);
    }
}
//# sourceMappingURL=updateActionItemTool.js.map