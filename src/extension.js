...
import { handleUri, registerCommands, registerCodeActions, registerTerminalActions, CodeActionProvider, } from "./activate";
...
export async function activate(context) {
    ...
    const defaultCommands = vscode.workspace.getConfiguration(Package.name).get("allowedCommands") || [];
        ...
        for (const folder of vscode.workspace.workspaceFolders) {
            ...
            if (manager) {
                ...
                try {
                    ...
                    outputChannel.appendLine(`[CodeIndexManager] Error during background CodeIndexManager configuration/indexing for ${folder.uri.fsPath}: ${error.message || error}`);
                }
    ...
    authStateChangedHandler = async (data) => {
        ...
        if (data.state === "logged-out") {
            try {
                ...
                cloudLogger(`[CloudService] Failed to disconnect BridgeOrchestrator on logout: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    };
    settingsUpdatedHandler = async () => {
        ...
        if (userInfo && CloudService.instance.cloudAPI) {
            try {
                ...
                const isCloudAgent = typeof process.env.ROO_CODE_CLOUD_TOKEN === "string" && process.env.ROO_CODE_CLOUD_TOKEN.length > 0;
                ...
                cloudLogger(`[CloudService] BridgeOrchestrator#connectOrDisconnect failed on settings change: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        postStateListener();
    };
    userInfoHandler = async ({ userInfo }) => {
        postStateListener();
        ...
        try {
            ...
            cloudLogger(`[CloudService] BridgeOrchestrator#connectOrDisconnect failed on user change: ${error instanceof Error ? error.message : String(error)}`);
        }
    };
    ...
    try {
        ...
        outputChannel.appendLine(`[CloudService] Failed to register TelemetryClient: ${error instanceof Error ? error.message : String(error)}`);
    }
    // Add to subscriptions for proper cleanup on deactivate.
    context.subscriptions.push(cloudService);
    ...
    try {
        ...
        outputChannel.appendLine(`[CloudService] Failed to initialize cloud profile sync: ${error instanceof Error ? error.message : String(error)}`);
    }
    // Finish initializing the provider.
    TelemetryService.instance.setProvider(provider);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(ClineProvider.sideBarId, provider, {
    ...
    if (!context.globalState.get("firstInstallCompleted")) {
        ...
        try {
            ...
            await vscode.commands.executeCommand("workbench.action.openWalkthrough", "kilocode.kilo-code#kiloCodeWalkthrough", false);
        }
        catch (error) {
    ...
    const dumpCodexInfoDisposable = vscode.commands.registerCommand("golden-workplace.dev.dumpCodexInfo", async () => {
        ...
        const codexCommands = commands.filter((command) => command.startsWith("chatgpt.") || command.includes("codex") || command.includes("openai"));
    ...
    if (process.env.NODE_ENV === "development") {
        ...
        console.log(`♻️♻️♻️ Core auto-reloading: Watching for changes in ${watchPaths.map(({ path }) => path).join(", ")}`);
...
export async function deactivate() {
    ...
    if (cloudService && CloudService.hasInstance()) {
        try {
            ...
            outputChannel.appendLine(`Failed to clean up CloudService event handlers: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
