 ...
 */
export async function migrateSettings(context, outputChannel) {
        ...
        try {
            for (const migration of fileMigrations) {
                ...
                if (oldFileExists && !newFileExists) {
                    ...
                    outputChannel.appendLine(`Skipping migration of ${migration.oldName} to ${migration.newName}: ${oldFileExists ? "new file already exists" : "old file not found"}`);
                }
            }
...
async function migrateCustomModesToYaml(settingsDir, outputChannel) {
    ...
    try {
        ...
        try {
            ...
            outputChannel.appendLine("Successfully migrated custom_modes.json to YAML format (original JSON file preserved for rollback purposes)");
        }
        catch (parseError) {
            // Handle corrupt JSON file
            outputChannel.appendLine(`Error parsing custom_modes.json: ${parseError}. File might be corrupted. Skipping migration.`);
        }
    }
    catch (fileError) {
...
async function migrateDefaultCommands(context, outputChannel) {
    try {
        ...
        if (filteredCommands.length < originalLength) {
            ...
            outputChannel.appendLine(`[Default Commands Migration] Removed ${removedCount} command(s) from old defaults to prevent arbitrary code execution vulnerability`);
        }
        else {
