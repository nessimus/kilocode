...
import { checkGitInstalled, searchCommits, getCommitInfo, getWorkingState, getGitRepositoryInfo, sanitizeGitUrl, extractRepositoryName, getWorkspaceGitInfo, convertGitUrlToHttps, } from "../git";
...
describe("git utils", () => {
    ...
    describe("checkGitInstalled", () => {
        it("should return true when git --version succeeds", async () => {
            vitest.mocked(exec).mockImplementation((command, options, callback) => {
    ...
    describe("searchCommits", () => {
        ...
        it("should return commits when git is installed and repo exists", async () => {
            ...
            expect(vitest.mocked(exec)).toHaveBeenCalledWith('git log -n 10 --format="%H%n%h%n%s%n%an%n%ad" --date=short --grep="test" --regexp-ignore-case', { cwd }, expect.any(Function));
        });
