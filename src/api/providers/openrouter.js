...
import { openRouterDefaultModelId, openRouterDefaultModelInfo, OPEN_ROUTER_PROMPT_CACHING_MODELS, DEEP_SEEK_DEFAULT_TEMPERATURE, } from "@roo-code/types";
...
export class OpenRouterHandler extends BaseProvider {
    ...
    async generateImage(prompt, model, apiKey, inputImage, taskId) {
        ...
        try {
            const response = await fetch(`${this.options.openRouterBaseUrl || "https://openrouter.ai/api/v1/"}chat/completions`, // kilocode_change: support baseUrl
            {
                method: "POST",
                headers: {
                    // kilocode_change start
