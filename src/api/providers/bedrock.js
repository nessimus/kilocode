...
import { bedrockDefaultModelId, bedrockModels, bedrockDefaultPromptRouterModelId, BEDROCK_DEFAULT_TEMPERATURE, BEDROCK_MAX_TOKENS, BEDROCK_DEFAULT_CONTEXT, AWS_INFERENCE_PROFILE_MAPPING, BEDROCK_CLAUDE_SONNET_4_MODEL_ID, } from "@roo-code/types";
 ...
 *
 *************************************************************************************/
export class AwsBedrockHandler extends BaseProvider {
    options;
    client;
    arnInfo;
    constructor(options) {
        ...
        if (this.options.awsCustomArn) {
            ...
            if (!this.arnInfo.isValid) {
                ...
                const errorMessage = this.arnInfo.errorMessage ||
    ...
    async *createMessage(systemPrompt, messages, metadata) {
        ...
        const formatted = this.convertToBedrockConverseMessages(messages, systemPrompt, usePromptCache, modelConfig.info, conversationId);
        ...
        const isThinkingEnabledBySettings = shouldUseReasoningBudget({ model: modelConfig.info, settings: this.options }) &&
    ...
    async completePrompt(prompt) {
        try {
            ...
            const thinkingEnabled = shouldUseReasoningBudget({ model: modelConfig.info, settings: this.options }) &&
                modelConfig.reasoning &&
                modelConfig.reasoningBudget;
            const inferenceConfig = {
                maxTokens: modelConfig.maxTokens || modelConfig.info.maxTokens,
            ...
            const payload = {
                ...
                ], undefined, false, modelConfig.info, conversationId).messages,
                inferenceConfig,
            };
     ...
     */
    convertToBedrockConverseMessages(anthropicMessages, systemMessage, usePromptCache = false, modelInfo, conversationId) {
        // First convert messages using shared converter for proper image handling
        const convertedMessages = sharedConverter(anthropicMessages);
        ...
        const previousPlacements = conversationId && this.previousCachePointPlacements[conversationId]
...
- Model ID is correct for the requested features`,
    ...
    formatErrorMessage(error, errorType, _isStreamContext) {
        ...
        const region = typeof this?.client?.config?.region === "function"
