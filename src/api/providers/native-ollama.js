    ...
    for (const anthropicMessage of anthropicMessages) {
        if (typeof anthropicMessage.content === "string") {
            ...
            if (anthropicMessage.role === "user") {
                ...
                const { nonToolMessages } = anthropicMessage.content.reduce((acc, part) => {
...
export class NativeOllamaHandler extends BaseProvider {
    options;
    client;
    models = {};
    isInitialized = false; // kilocode_change
    constructor(options) {
        super();
    ...
    async *createMessage(systemPrompt, messages, metadata) {
        ...
        if (modelInfo.maxTokens && estimatedTokenCount > modelInfo.maxTokens) {
            throw new Error(`Input message is too long for the selected model. Estimated tokens: ${estimatedTokenCount}, Max tokens: ${modelInfo.maxTokens}. To increase the context window size, see: https://kilocode.ai/docs/providers/ollama#configure-the-context-size`);
        }
        // kilocode_change end
        ...
        try {
            ...
            if (error.code === "ECONNREFUSED") {
                throw new Error(`Ollama service is not running at ${this.options.ollamaBaseUrl || "http://localhost:11434"}. Please start Ollama first.`);
            }
            else if (statusCode === 404) {
                throw new Error(`Model ${this.getModel().id} not found in Ollama. Please pull the model first with: ollama pull ${this.getModel().id}`);
            }
