...
import { anthropicDefaultModelId, anthropicModels, ANTHROPIC_DEFAULT_MAX_TOKENS, } from "@roo-code/types";
    ...
    async *createMessage(systemPrompt, messages, metadata) {
        ...
        if (inputTokens > 0 || outputTokens > 0 || cacheWriteTokens > 0 || cacheReadTokens > 0) {
            yield {
                ...
                inputTokens: 0,
                outputTokens: 0,
                totalCost: calculateApiCostAnthropic(this.getModel().info, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens),
            };
        }
    }
    getModel() {
        const modelId = this.options.apiModelId;
        let id = modelId && modelId in anthropicModels ? modelId : anthropicDefaultModelId;
