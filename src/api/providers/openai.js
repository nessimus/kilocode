...
import { azureOpenAiDefaultApiVersion, openAiModelInfoSaneDefaults, DEEP_SEEK_DEFAULT_TEMPERATURE, OPENAI_AZURE_AI_INFERENCE_PATH, } from "@roo-code/types";
...
export class OpenAiHandler extends BaseProvider {
    ...
    async *createMessage(systemPrompt, messages, metadata) {
        ...
        if (this.options.openAiStreamingEnabled ?? true) {
            ...
            let stream;
            try {
                stream = await this.client.chat.completions.create(requestOptions, isAzureAiInference ? { path: OPENAI_AZURE_AI_INFERENCE_PATH } : {});
            }
            catch (error) {
            ...
            let response;
            try {
                response = await this.client.chat.completions.create(requestOptions, this._isAzureAiInference(modelUrl) ? { path: OPENAI_AZURE_AI_INFERENCE_PATH } : {});
            }
            catch (error) {
                throw handleOpenAIError(error, this.providerName);
            }
    ...
    async completePrompt(prompt) {
        try {
            ...
            let response;
            try {
                response = await this.client.chat.completions.create(requestOptions, isAzureAiInference ? { path: OPENAI_AZURE_AI_INFERENCE_PATH } : {});
            }
            catch (error) {
                throw handleOpenAIError(error, this.providerName);
            }
    ...
    async *handleO3FamilyMessage(modelId, systemPrompt, messages) {
        ...
        if (this.options.openAiStreamingEnabled ?? true) {
            ...
            let stream;
            try {
                stream = await this.client.chat.completions.create(requestOptions, methodIsAzureAiInference ? { path: OPENAI_AZURE_AI_INFERENCE_PATH } : {});
            }
            catch (error) {
                throw handleOpenAIError(error, this.providerName);
            }
            ...
            let response;
            try {
                response = await this.client.chat.completions.create(requestOptions, methodIsAzureAiInference ? { path: OPENAI_AZURE_AI_INFERENCE_PATH } : {});
            }
            catch (error) {
                throw handleOpenAIError(error, this.providerName);
            }
