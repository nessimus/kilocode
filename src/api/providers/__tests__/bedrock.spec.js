...
describe("AwsBedrockHandler", () => {
    ...
    describe("getModel", () => {
        ...
        it("should use default prompt router model when prompt router arn is entered but no model can be identified from the ARN", () => {
            const customArnHandler = new AwsBedrockHandler({
                awsCustomArn: "arn:aws:bedrock:ap-northeast-3:123456789012:default-prompt-router/my_router_arn_no_model",
                awsAccessKey: "test-access-key",
                awsSecretKey: "test-secret-key",
                awsRegion: "us-east-1",
            });
            const modelInfo = customArnHandler.getModel();
            expect(modelInfo.id).toBe("arn:aws:bedrock:ap-northeast-3:123456789012:default-prompt-router/my_router_arn_no_model");
            expect(modelInfo.info).toBeDefined();
    ...
    describe("region mapping and cross-region inference", () => {
        describe("getPrefixForRegion", () => {
            it("should return correct prefix for US regions", () => {
                ...
                const getPrefixForRegion = AwsBedrockHandler.getPrefixForRegion;
        ...
        describe("isSystemInferenceProfile", () => {
            it("should return true for AWS inference profile prefixes", () => {
                const isSystemInferenceProfile = AwsBedrockHandler.isSystemInferenceProfile;
        ...
        describe("parseBaseModelId", () => {
            it("should remove defined inference profile prefixes", () => {
                ...
                expect(parseBaseModelId("us.anthropic.claude-3-5-sonnet-20241022-v2:0")).toBe("anthropic.claude-3-5-sonnet-20241022-v2:0");
                expect(parseBaseModelId("eu.anthropic.claude-3-haiku-20240307-v1:0")).toBe("anthropic.claude-3-haiku-20240307-v1:0");
                expect(parseBaseModelId("apac.anthropic.claude-3-opus-20240229-v1:0")).toBe("anthropic.claude-3-opus-20240229-v1:0");
            });
            it("should not modify model IDs without defined prefixes", () => {
                ...
                const parseBaseModelId = handler.parseBaseModelId.bind(handler);
                expect(parseBaseModelId("anthropic.claude-3-5-sonnet-20241022-v2:0")).toBe("anthropic.claude-3-5-sonnet-20241022-v2:0");
            ...
            it("should not modify model IDs with other prefixes", () => {
                ...
                expect(parseBaseModelId("ap.anthropic.claude-3-5-sonnet-20241022-v2:0")).toBe("ap.anthropic.claude-3-5-sonnet-20241022-v2:0");
                expect(parseBaseModelId("apne1.anthropic.claude-3-5-sonnet-20241022-v2:0")).toBe("apne1.anthropic.claude-3-5-sonnet-20241022-v2:0");
                expect(parseBaseModelId("use1.anthropic.claude-3-5-sonnet-20241022-v2:0")).toBe("use1.anthropic.claude-3-5-sonnet-20241022-v2:0");
            });
        });
        ...
        describe("ARN parsing with inference profiles", () => {
            it("should detect cross-region inference from ARN model ID", () => {
                ...
                const result = parseArn("arn:aws:bedrock:us-east-1:123456789012:foundation-model/us.anthropic.claude-3-5-sonnet-20241022-v2:0");
            ...
            it("should not detect cross-region inference for non-prefixed models", () => {
                ...
                const parseArn = handler.parseArn.bind(handler);
                const result = parseArn("arn:aws:bedrock:us-east-1:123456789012:foundation-model/anthropic.claude-3-5-sonnet-20241022-v2:0");
                expect(result.isValid).toBe(true);
            ...
            it("should detect cross-region inference for defined prefixes", () => {
                ...
                const parseArn = handler.parseArn.bind(handler);
                const euResult = parseArn("arn:aws:bedrock:eu-west-1:123456789012:foundation-model/eu.anthropic.claude-3-5-sonnet-20241022-v2:0");
                const apacResult = parseArn("arn:aws:bedrock:ap-southeast-1:123456789012:foundation-model/apac.anthropic.claude-3-5-sonnet-20241022-v2:0");
            ...
            it("should not detect cross-region inference for other prefixes", () => {
                ...
                const result = parseArn("arn:aws:bedrock:us-east-1:123456789012:foundation-model/ap.anthropic.claude-3-5-sonnet-20241022-v2:0");
                expect(result.crossRegionInference).toBe(false);
...
//# sourceMappingURL=bedrock.spec.js.map