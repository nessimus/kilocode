...
describe("OpenAiNativeHandler", () => {
    ...
    describe("GPT-5 models", () => {
        it("should handle GPT-5 model with Responses API", async () => {
            ...
            const mockFetch = vitest.fn().mockResolvedValue({
                ...
                body: new ReadableStream({
                    start(controller) {
                        ...
                        controller.enqueue(new TextEncoder().encode('data: {"type":"response.created","response":{"id":"test","status":"in_progress"}}\n\n'));
            ...
            expect(mockFetch).toHaveBeenCalledWith("https://api.openai.com/v1/responses", expect.objectContaining({
                method: "POST",
        ...
        it("should support minimal reasoning effort for GPT-5", async () => {
            ...
            const mockFetch = vitest.fn().mockResolvedValue({
                ...
                body: new ReadableStream({
                    start(controller) {
                        controller.enqueue(new TextEncoder().encode('data: {"type":"response.output_item.added","item":{"type":"text","text":"Minimal effort"}}\n\n'));
                        controller.enqueue(new TextEncoder().encode("data: [DONE]\n\n"));
                        controller.close();
                    },
                }),
            ...
            const mockFetch = vitest.fn().mockResolvedValue({
                ...
                body: new ReadableStream({
                    start(controller) {
                        ...
                        controller.enqueue(new TextEncoder().encode('data: {"type":"response.created","response":{"id":"resp_123","status":"in_progress"}}\n\n'));
            ...
            const mockFetch = vitest.fn().mockResolvedValue({
                ...
                body: new ReadableStream({
                    start(controller) {
                        ...
                        controller.enqueue(new TextEncoder().encode('data: {"type":"response.audio.delta","delta":"..."}\n\n'));
            ...
            const mockFetch = vitest
                ...
                body: new ReadableStream({
                    start(controller) {
                        ...
                        controller.enqueue(new TextEncoder().encode('data: {"type":"response.done","response":{"id":"resp_789","output":[{"type":"text","content":[{"type":"text","text":"First"}]}],"usage":{"prompt_tokens":10,"completion_tokens":1}}}\n\n'));
                        controller.enqueue(new TextEncoder().encode("data: [DONE]\n\n"));
                        controller.close();
                    },
                }),
                ...
                body: new ReadableStream({
                    start(controller) {
                        // Second response
                        controller.enqueue(new TextEncoder().encode('data: {"type":"response.output_item.added","item":{"type":"text","text":"Second"}}\n\n'));
                        controller.enqueue(new TextEncoder().encode("data: [DONE]\n\n"));
                        controller.close();
                    },
                }),
        ...
        it("should only send latest message when using previous_response_id", async () => {
            ...
            const mockFetch = vitest
                ...
                body: new ReadableStream({
                    start(controller) {
                        // First response with ID
                        controller.enqueue(new TextEncoder().encode('data: {"type":"response.done","response":{"id":"resp_001","output":[{"type":"text","content":[{"type":"text","text":"First"}]}],"usage":{"prompt_tokens":50,"completion_tokens":1}}}\n\n'));
                        controller.enqueue(new TextEncoder().encode("data: [DONE]\n\n"));
                        controller.close();
                    },
                }),
...
describe("GPT-5 streaming event coverage (additional)", () => {
    ...
    it("should handle reasoning delta events for GPT-5", async () => {
        const mockFetch = vitest.fn().mockResolvedValue({
            ...
            body: new ReadableStream({
                start(controller) {
                    controller.enqueue(new TextEncoder().encode('data: {"type":"response.reasoning.delta","delta":"Thinking about the problem..."}\n\n'));
        ...
        const mockFetch = vitest.fn().mockResolvedValue({
            ...
            body: new ReadableStream({
                start(controller) {
                    ...
                    // Malformed JSON line
                    controller.enqueue(new TextEncoder().encode('data: {"type":"response.text.delta","delta":"Bad"\n\n'));
                    // Valid line after malformed
                    controller.enqueue(new TextEncoder().encode('data: {"type":"response.output_item.added","item":{"type":"text","text":"After"}}\n\n'));
                    controller.enqueue(new TextEncoder().encode("data: [DONE]\n\n"));
                    controller.close();
                },
            }),
    ...
    describe("Codex Mini Model", () => {
        ...
        it("should handle codex-mini-latest with multiple user messages", async () => {
            ...
            const mockFetch = vitest.fn().mockResolvedValue({
                ...
                body: new ReadableStream({
                    start(controller) {
                        controller.enqueue(new TextEncoder().encode('data: {"type":"response.output_text.delta","delta":"Combined response"}\n\n'));
...
//# sourceMappingURL=openai-native.spec.js.map