...
describe("GeminiCliHandler", () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(handler, "discoverProjectId").mockResolvedValue("test-project-123");
    });
    ...
    describe("OAuth authentication", () => {
        ...
        it("should refresh expired tokens", async () => {
            ...
            fs.readFile.mockResolvedValueOnce(JSON.stringify(expiredCredentials));
            await handler["ensureAuthenticated"]();
            expect(handler["authClient"].refreshAccessToken).toHaveBeenCalled();
            expect(fs.writeFile).toHaveBeenCalledWith(expect.stringMatching(/\.gemini[/\\]oauth_creds\.json$/), expect.stringContaining("refreshed-token"));
        });
