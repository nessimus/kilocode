...
export const hasLoadedFullDetails = (modelId) => {
    return modelsWithLoadedDetails.has(modelId);
};
export const forceFullModelDetailsLoad = async (baseUrl, modelId) => {
    try {
        ...
        if (error.code === "ECONNREFUSED") {
            ...
            console.error(`Error refreshing LMStudio model details: ${JSON.stringify(error, Object.getOwnPropertyNames(error), 2)}`);
        }
    }
};
export const parseLMStudioModel = (rawModel) => {
    ...
    const modelInfo = Object.assign({}, lMStudioDefaultModelInfo, {
    ...
    try {
        ...
        for (const lmstudioModel of loadedModels) {
            ...
            const existingKey = Object.keys(models).find((key) => {
                ...
                return (keyLower.includes(`/${loadedModelId}/`) ||
