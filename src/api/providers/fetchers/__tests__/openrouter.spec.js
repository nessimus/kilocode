...
import { OPEN_ROUTER_PROMPT_CACHING_MODELS, OPEN_ROUTER_COMPUTER_USE_MODELS, OPEN_ROUTER_REASONING_BUDGET_MODELS, OPEN_ROUTER_REQUIRED_REASONING_BUDGET_MODELS, } from "@roo-code/types";
...
describe("OpenRouter API", () => {
    describe("getOpenRouterModels", () => {
        ...
        it.skip("fetches models and validates schema", async () => {
            ...
            const ourCachingModels = Array.from(OPEN_ROUTER_PROMPT_CACHING_MODELS).filter((id) => !excludedModels.has(id));
            ...
            const expectedComputerUseModels = Array.from(OPEN_ROUTER_COMPUTER_USE_MODELS)
                ...
                .sort();
            expect(Object.entries(models)
                .filter(([_, model]) => model.supportsComputerUse)
                .map(([id, _]) => id)
                .sort()).toEqual(expectedComputerUseModels);
            expect(Object.entries(models)
            ...
            expect(Object.entries(models)
                ...
                .map(([id, _]) => id)
                .sort()).toEqual(expectedReasoningBudgetModels);
