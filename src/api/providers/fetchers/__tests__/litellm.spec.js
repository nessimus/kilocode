...
describe("getLiteLLMModels", () => {
    ...
    it("throws error for unexpected response format", async () => {
        ...
        mockedAxios.get.mockResolvedValue(mockResponse);
        await expect(getLiteLLMModels("test-api-key", "http://localhost:4000")).rejects.toThrow("Failed to fetch LiteLLM models: Unexpected response format.");
    });
    it("throws detailed error for HTTP error responses", async () => {
        ...
        await expect(getLiteLLMModels(DUMMY_INVALID_KEY, "http://localhost:4000")).rejects.toThrow("Failed to fetch LiteLLM models: 401 Unauthorized. Check base URL and API key.");
    });
    it("throws network error for request failures", async () => {
        ...
        mockedAxios.isAxiosError.mockReturnValue(true);
        mockedAxios.get.mockRejectedValue(axiosError);
        await expect(getLiteLLMModels("test-api-key", "http://invalid-url")).rejects.toThrow("Failed to fetch LiteLLM models: No response from server. Check LiteLLM server status and base URL.");
    });
    ...
    it("handles timeout parameter correctly", async () => {
        ...
        mockedAxios.get.mockResolvedValue(mockResponse);
        await getLiteLLMModels("test-api-key", "http://localhost:4000");
        expect(mockedAxios.get).toHaveBeenCalledWith("http://localhost:4000/v1/model/info", expect.objectContaining({
            timeout: 5000,
        }));
    });
...
//# sourceMappingURL=litellm.spec.js.map