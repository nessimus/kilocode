 ...
 * <mcreference link="https://docs.io.net/reference/get-started-with-io-intelligence-api" index="1">1</mcreference>
 */
export async function getIOIntelligenceModels(apiKey) {
    const now = Date.now();
    // Check cache
    ...
    try {
        ...
        const response = await axios.get("https://api.intelligence.io.solutions/api/v1/models", {
            headers,
 ...
 */
export function getCachedIOIntelligenceModels() {
    return cache?.data || null;
}
/**
 * Clear the cache
 */
export function clearIOIntelligenceCache() {
    cache = null;
}
