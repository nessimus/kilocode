...
describe("DiffViewProvider", () => {
    ...
    describe("open method", () => {
        it("should pre-open file as text document before executing diff command", async () => {
            ...
            vi.mocked(vscode.workspace.onDidOpenTextDocument).mockImplementation((callback) => {
                ...
                setTimeout(() => {
                    callback({ uri: { fsPath: `${mockCwd}/test.md` } });
                }, 0);
            ...
            expect(vscode.window.showTextDocument).toHaveBeenCalledWith(expect.objectContaining({ fsPath: `${mockCwd}/test.md` }), { preview: false, viewColumn: vscode.ViewColumn.Active, preserveFocus: true });
            // Verify that the diff command was executed
            expect(vscode.commands.executeCommand).toHaveBeenCalledWith("vscode.diff", expect.any(Object), expect.any(Object), `test.md: ${DIFF_VIEW_LABEL_CHANGES} (Editable)`, { preserveFocus: true });
        });
        it("should handle showTextDocument failure", async () => {
            ...
            await expect(diffViewProvider.open("test.md")).rejects.toThrow("Failed to execute diff command for /mock/cwd/test.md: Cannot open file");
        });
    });
    describe("closeAllDiffViews method", () => {
        it("should close diff views including those identified by label", async () => {
            ...
            expect(closedTabs.find((t) => t.label === `file4.ts: ${DIFF_VIEW_LABEL_CHANGES} (Editable)` && t.isDirty)).toBeUndefined();
        });
    });
    describe("saveDirectly method", () => {
        ...
        it("should write content directly to file without opening diff view", async () => {
            ...
            expect(vscode.window.showTextDocument).toHaveBeenCalledWith(expect.objectContaining({ fsPath: `${mockCwd}/test.ts` }), { preview: false, preserveFocus: true });
...
//# sourceMappingURL=DiffViewProvider.spec.js.map