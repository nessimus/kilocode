...
// Mock path module
const mockPathJoin = vi.fn((dir, filename) => `${dir}/${filename}`);
...
let mockReadlineInterface = null;
vi.mock("readline", () => ({
    default: {
...
describe("runClaudeCode", () => {
    beforeEach(() => {
        ...
        mockPathJoin.mockImplementation((dir, filename) => `${dir}/${filename}`);
        // kilocode_change end
    ...
    test("should handle platform-specific stdin behavior", async () => {
        ...
        expect(mockWriteFile).toHaveBeenCalledWith(expect.stringContaining("kilocode-system-prompt-"), systemPrompt, "utf8");
        // kilocode_change end
    ...
    test("should handle stdin access errors gracefully", async () => {
        ...
        mockProcessWithoutStdin.stdin = null;
        // Mock console.error to verify error logging
        const consoleErrorSpy = vi.spyOn(console, "error").mockImplementation(() => { });
    ...
    test("should handle ENOENT errors during process execution with helpful error message", async () => {
        ...
        vi.mocked(readline.default.createInterface).mockReturnValueOnce(mockReadlineForError);
        mockExeca.mockReturnValueOnce(mockProcessWithError);
        const options = {
            systemPrompt: "You are a helpful assistant",
            messages: [{ role: "user", content: "Hello" }],
