...
describe("TerminalProcess with Bash Command Output", () => {
    ...
    it.skip(`should execute 'yes AAA... | head -n ${1_000_000}' and verify lines of 'A's`, async () => {
        ...
        const { executionTimeUs, capturedOutput } = await testTerminalCommand(`yes "${"A".repeat(76)}" | head -n ${TEST_LINES}`, expectedOutput);
        console.log(`'yes "${"A".repeat(76)}" | head -n ${TEST_LINES}' execution time: ${executionTimeUs} microseconds (${executionTimeUs / 1000} milliseconds)`);
...
//# sourceMappingURL=TerminalProcessExec.bash.spec.js.map