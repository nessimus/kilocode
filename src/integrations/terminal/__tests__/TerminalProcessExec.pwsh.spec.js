...
describePlatform("TerminalProcess with PowerShell Command Output", () => {
    ...
    it(TEST_PURPOSES.LARGE_OUTPUT, async () => {
        ...
        const expectedOutput = Array.from({ length: lines }, (_, i) => `${TEST_TEXT.LARGE_PREFIX}${i + 1}`).join("\n") + "\n";
        // Skip the automatic output verification
        const skipVerification = true;
        const { executionTimeUs, capturedOutput } = await testPowerShellCommand(command, expectedOutput, false, skipVerification);
...
//# sourceMappingURL=TerminalProcessExec.pwsh.spec.js.map